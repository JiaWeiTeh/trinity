...
| Prepend t=0 | 263-274 | All arrays get `np.insert(..., 0, arr[0])` so interpolators are defined down to t=0. |
...
- Line 263-331: `get_filename` → `opiate_cooling_{rot|norot}_Z{1.00|0.15}_age{age}.dat`.
...
- [ ] `t=0` prepend (currently `read_SB99.py:263-274`) detects existing t=0
...
absent. Affects only the SN scaling block (`read_SB99.py:229-247`).
...
The harness internally:

1. For each anchor, copies the `.param` to a temp location, rewrites
   `path2output` to point under `analysis/sb99-refactor-golden/<stem>/`.
2. Invokes `python run.py <tmp_param>` via `subprocess.run`.
3. After completion, pickles the loader's 11-array output and the 10
   interpolators evaluated at a dense time grid (so unit-layer tests don't
   need to re-run trinity).
...
| Path-resolution drift due to subtle formatting in `get_filename` (mantissa formatter `format_e` at `read_SB99.py:327-333`) | Medium | High | Path-resolution matrix test covers all legal combos. |
...
| `t=0` prepend hack (loader 263-274) double-applied if generic CSV already has t=0 | Medium | Medium | PR-2 loader detects `t[0] == 0` and skips prepend; explicit `test_t0_prepend_idempotent`. |
