...
| `path_cooling_nonCIE` | `src/_input/default.param:319` | `def_dir` | Directory of OPIATE cubes; resolves to `lib/cooling/opiate/` (`read_param.py:417`). Used by `read_cloudy.get_filename()` to build per-age filenames. |
...
- `path_cooling_nonCIE = def_dir` continues to resolve to
  `lib/cooling/opiate/` and the OPIATE filename grammar continues to
...
2. **Loader-layer capture (in-process).** Import
   ...
   `lib/cooling/opiate/` for the resolved
...
3. **Anchor configs adequacy.** The three `cloud_example_*` configs
   ...
   cubes exist in `lib/cooling/opiate/`).
...
- **Footprint.** Three Python files contain non-trivial loader code:
  ...
  - `src/cooling/net_coolingcurve.py` (166 LOC) — consumer; touches
    constants only.
- **Default cooling library paths.** `lib/cooling/opiate/` and
  `lib/cooling/CIE/*.dat` (the `lib/` tree is gitignored).
