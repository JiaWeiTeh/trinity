...
def plot_grid(folder_path, output_dir=None, ndens_filter=None,
    ...
    """
    Plot grid of cancellation metric from simulations in a folder.
    ...
    for ndens in ndens_to_plot:
        ...
        # Save figure to ./fig/{folder_name}/cancellation_{ndens_tag}.pdf
        fig_dir = Path(output_dir) if output_dir else FIG_DIR / folder_name
        fig_dir.mkdir(parents=True, exist_ok=True)
        out_pdf = fig_dir / f"cancellation_{ndens_tag}.pdf"
        fig.savefig(out_pdf, bbox_inches="tight")
        print(f"  Saved: {out_pdf}")
...
Examples:
  ...
  python paper_cancellationMetric.py -F /path/to/simulations/ -n 1e4
        """
    )
    parser.add_argument(
