...
def plot_grid(folder_path, output_dir=None, ndens_filter=None):
    ...
    for ndens in ndens_to_plot:
        ...
        fig_dir.mkdir(parents=True, exist_ok=True)
        out_pdf = fig_dir / f"{folder_name}_{ndens_tag}_force.pdf"
        fig.savefig(out_pdf, bbox_inches="tight")
        print(f"  Saved: {out_pdf}")
...
Examples:
  ...
  python paper_forceFraction.py -F /path/to/simulations/
  python paper_forceFraction.py -F /path/to/simulations/ -n 1e4  # filter by density
        """
    )
    parser.add_argument(
