...
Background: see docs/dev/archive/sb99-refactor-audit.md, sections 9 (legacy as
...
# sps_col_* lines. See docs/dev/archive/sb99-refactor-audit.md §9 for background.
#
...
def validate_user_column_map(column_map: Dict[str, ColumnSpec], sps_path: str) -> None:
    ...
    fillable template on failure -- see docs/dev/archive/sb99-refactor-audit.md §10
    PR-2.
...
def validate_t_monotonic(t: np.ndarray, filepath: str) -> None:
    ...
    Applied by _read_sps_user in trinity/sps/read_sps.py.
    """
    if len(t) < 2:
