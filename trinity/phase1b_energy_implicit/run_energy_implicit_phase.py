...
def compute_forces_pure(
    ...
    P_drive = P_HII if front_branch else max(Pb, P_HII)
...
def run_phase_energy(params) -> ImplicitPhaseResults:
    ...
    # Consecutive no-physical-root streak (the frozen-implicit signature:
    ...
    while t_now <= tmax and segment_count < MAX_SEGMENTS:
        ...
        if bubble_props is not None:
            ...
            # for monitoring; NOT used in any physics. docs/dev/archive/betadelta/stalling-energy-phase.md
            ...
            # freeze-watch: grep-able approach trace for the dMdt->0 crossing
            # (KAPPA_FREEZE_MECHANISM.md). DEBUG-only, one line per segment.
            logger.debug(
                f"freeze-watch: segment={segment_count} t={t_now:.6e} "
                f"dMdt={params['bubble_dMdt'].value:.6e} "
                f"Lloss/Lgain={params['bubble_Lloss'].value / max(params['bubble_Lgain'].value, 1e-300):.4f}"
            )
    ...
    # (Pb ~ -1.6e18). The last in-loop snapshot already holds the last healthy
    # state, and the ENERGY_COLLAPSED code/reason propagate independently of this
    # block (set before the break above). See docs/dev/transition/pdv-trigger/
    ...
    if termination_reason != "energy_collapsed":
        ...
        # Energy-collapsed: skip the Pb recompute (Eb<0 -> garbage), but still persist a
        # final snapshot so the ENERGY_COLLAPSED code/reason reach the output. params already
        # holds the last healthy Pb and derived state (set in-loop before the collapse), so
        # this records the true stop fate with a finite, positive terminal Pb.
        params.save_snapshot()
    ...
    if no_root_streak > 0:
        completion_log(
            f"  Phase ended during a {no_root_streak}-segment no-physical-root "
            f"streak (since t={no_root_streak_t0:.6e} Myr): the held state is "
            f"frozen, not solved — see KAPPA_FREEZE_MECHANISM.md"
        )
