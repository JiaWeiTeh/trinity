...
# (``python trinity/_output/cloudy/trinity_to_cloudy.py ...``), Python only puts
# the script's own directory on sys.path, so the ``from trinity._output...``
# imports below fail. Detect the file-as-script case and prepend the repo
# root (three levels up: cloudy/ → _output/ → trinity/ → repo root) so the
...
from trinity._output.cloudy.dlaw import DEFAULT_MIN_ROWS, DlawError
