...
SPECS: tuple[ParamSpec, ...] = (
    ...
    ParamSpec(name='transition_trigger', default='instantaneous', info="Implicit->momentum transition trigger. 'instantaneous' (default): terminate on the cooling-balance ratio (Lgain-Lloss)/Lgain < phaseSwitch_LlossLgain only; the blowout criterion R2>rCloud is logged (shadow) to transition_shadow.jsonl but not acted on. 'cooling_or_blowout' (not yet implemented; P-promote) would also transition on blowout.", category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True),
