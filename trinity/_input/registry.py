...
from trinity._input.fkappa_auto import resolve_fkappa_auto
...
def _validate_phase1a_segFrac(value, params) -> None:
    """Phase-1a segment fraction must be a number in [0, 1].

    ``dt_segment = phase1a_segFrac*(t_now - tSF)`` with the driving terms
    frozen across the segment.  0 is the documented fallback to the fixed
    30-yr ``SEGMENT_DURATION``, and ``run_energy_phase`` implements it by
    falling back whenever that product is <= 0 -- which silently swallows
    negatives too, so an unvalidated -0.1 would quietly run stock segments.
    Above 1 a single frozen segment spans more than the bubble's entire age,
    which is the defect this parameter exists to prevent
    (docs/dev/phase1a-init/FINDINGS.md)."""
    from trinity._input.errors import ParameterFileError
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ParameterFileError(
            f"Invalid phase1a_segFrac '{value}'. Must be a number in [0, 1]."
        )
    if not (0.0 <= value <= 1.0):
        raise ParameterFileError(
            f"Invalid phase1a_segFrac '{value}'. Must satisfy "
            f"0 <= phase1a_segFrac <= 1 (0 falls back to the fixed 30-yr "
            f"segment; > 1 freezes the driving terms for longer than the "
            f"bubble's whole age)."
        )
...
SPECS: tuple[ParamSpec, ...] = (
    ...
    ParamSpec(name='log_level', default='INFO', info='Logging level for terminal and file output. Controls how much detail is logged. DEBUG emits per-RHS-evaluation records (formatted and written to the log file on every solver stage), a measurable hot-path cost over a full run; INFO is the default, DEBUG is opt-in for diagnostics.', category='input_admin', unit=None, exclude_from_snapshot=True, run_const=True),
    ...
    ParamSpec(name='include_PHII', default='True', info='Include the photoionised-gas pressure P_HII in P_drive. When False, P_HII is set to zero. Since 2026-08-14 (C3c) P_HII is NOT the capped-Strömgren shell pressure it used to be — that quantity was an algebraic relabelling of the confining pressure, carrying no dependence on Qi. It is now a confinement regime switch on the CAVITY Strömgren density (get_bubbleParams.get_phii_c3c): exactly 0.0 while the ionised gas is confined, because a confined skin transmits the confining pressure rather than adding to it, and the cavity photoionised pressure once confinement cannot hold it. True and False differ only where the unconfined branch fires. At nominal wind that is rare before the momentum phase (P_C3a/Pb <= 0.04 in the energy and implicit phases of B3MW3 and B3MW10); at weak wind it fires inside the energy phase (B3MW001: from t = 1.2 kyr, and on every implicit row).', category='input_physical', unit=None, exclude_from_snapshot=True, run_const=True),
    ...
    ParamSpec(name='betadelta_solver', default='hybr', info="Energy-implicit (beta, delta) solver: 'hybr' (default; unbounded scipy root-finder with a physical dMdt>0 acceptance gate) or 'legacy' (bounded grid + L-BFGS-B).", category='input_admin', unit=None, exclude_from_snapshot=True, run_const=True, validator=_validate_betadelta_solver),
    ...
    ParamSpec(name='mu_atom', default='14/11', info='Mean mass per particle for neutral atomic gas (HI + He) = mu_H/(1+x_He). Derived at load from x_He (default 14/11 at x_He=0.1). In units of m_H.', category='input_constants', unit='m_H', exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='mu_ion', default='14/23', info='Mean mass per particle for the HOT bubble interior (fully ionised H+ + He2+) = mu_H/(2+x_He*(1+Z_He)). Derived at load from x_He, Z_He (default 14/23 at x_He=0.1, Z_He=2). The ~1e4 K ionised shell uses mu_ion_shell (singly-ionised He) instead. In units of m_H.', category='input_constants', unit='m_H', exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='mu_mol', default='14/6', info='Mean mass per particle for molecular gas (H2 + He) = mu_H/(0.5+x_He). Derived at load from x_He (default 14/6 at x_He=0.1). In units of m_H.', category='input_constants', unit='m_H', exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='mu_convert', default='1.4', info='Mass per hydrogen nucleus mu_H = (1+4*x_He) (=1.4 at x_He=0.1), in units of m_H. Use for the n_H -> rho mass conversion (rho = mu_convert * n_H); independent of ionisation state. Derived at load from x_He. All densities n in TRINITY are hydrogen-nuclei densities n_H.', category='input_constants', unit='m_H', exclude_from_snapshot=True, run_const=True),
    ...
    ParamSpec(name='transition_trigger', default='cooling_balance', info="Energy->momentum transition criteria, COMMA-SEPARATED (fires on whichever occurs first). Tokens: 'cooling_balance' (default = current behavior), 'blowout' (R2>rCloud), 'ebpeak' (net energy Edot_from_balance<=0); 'r1' aliases 'blowout,ebpeak'. e.g. 'cooling_balance,blowout'. Non-default sets DRIVE the R1 transition (docs/dev/transition/pt4/R1_SHADOW_PLAN.md).", category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='cooling_boost_mode', default='none', info="Opt-in unresolved-interface-cooling boost (Paper-II note, docs/dev/transition/pdv-trigger/PLAN.md). 'none' (default) = resolved cooling unchanged, BYTE-IDENTICAL. 'multiplier' = Lleak + cooling_boost_fmix*Lcool. 'theta_target' = max(Lcool+Lleak, cooling_boost_theta*Lmech) (tops up to a target loss fraction, single-count). The boosted loss feeds the beta-delta residual, the energy ODE, AND the transition trigger consistently. Never a (1-theta)*Lmech input rescale on top of Lcool -- that double-counts. FALLBACK: 'multiplier' is superseded for L21b *calibration* by cooling_boost_fA (docs/dev/transition/pdv-trigger/FA_STATE_COUPLED.md); retained as a supported opt-in mechanism and bench control arm.", category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='cooling_boost_fmix', default='1.0', info='Multiplier on the resolved cooling integral when cooling_boost_mode=multiplier (f_mix >= 1). Default 1.0 = no boost. FALLBACK: superseded for L21b *calibration* by cooling_boost_fA (docs/dev/transition/pdv-trigger/FA_STATE_COUPLED.md); retained pending a state-coupled successor.', category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True),
    ...
    ParamSpec(name='cooling_boost_kappa', default='1.0', info='Rung-A probe (docs/dev/transition/pdv-trigger/KAPPA_EFF_SCOPING.md): multiplier f_kappa on the Spitzer conduction coefficient C_thermal in the bubble-structure solve (f_kappa >= 1). Inflates conduction-zone cooling THROUGH the structure (theta emerges as an output), unlike the scalar cooling_boost on Lcool. Default 1.0 = byte-identical. NOTE it RAISES the evaporative mass flux, which MATCHES El-Badry+19 Eq 47 (mdot ~ C^(2/7), C=6e-7 cgs = C_thermal): measured 1.2175 vs analytic 2^(2/7)=1.2190 at f_kappa=2. [CORRECTED 2026-07-29, FINDINGS 23: an earlier note here called that the "wrong El-Badry sign" -- it is not; Eq 47 rises with conduction. the OTHER factor of Eq 47, (1-theta)^(37/35)/theta^(2/7), falls with theta and is the channel cooling_boost_fA moves.] It remains a structural probe rather than the final model for independent reasons: no whole-band f_kappa exists (FINDINGS 12) and a scalar on Spitzer C cannot represent cool-layer mixing, where kappa_mix/kappa_Spitzer ~ 1e3-1e7 (FINDINGS 9b). Set to \'auto\' to resolve at load time to the measured f_kappa that fires the cooling_balance trigger for this (mCloud, sfe, nCore), from the 819-run sweep grid (trinity/_input/fkappa_auto.py; docs/dev/transition/pdv-trigger/data/fkappa_nH_sweep.csv). The \'auto\' grid was measured at cooling_boost_fA=1; combining with f_A!=1 silently invalidates the lookup.', category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True, resolver=resolve_fkappa_auto),
    ParamSpec(name='cooling_boost_fA', default='1.0', info='Interface source-term boost f_A (docs/dev/transition/pdv-trigger/SOURCE_TERM_DESIGN.md): multiplies the net radiative dudt inside the bubble-structure ODE and the resolved L2+L3 loss integrals, in the interface band T < 10^5.5 K -- the bubble-side conduction front (zones L2+L3) draped on the contact discontinuity at R2, NOT the interface surface itself. The dudt gate is strictly T < 10^5.5 K; the L2+L3 component scaling is unconditional (a degenerate no-conduction-zone slice can scale a sliver above-band -- FINDINGS.md 15j). The 1-D projection of fractal-interface mixing (Lancaster) on the SOURCE side: cooling rises THROUGH the structure and evaporation dMdt FALLS -- the theta-channel of El-Badry Eq 47, (1-theta)^(37/35)/theta^(2/7). [CORRECTED 2026-07-29, FINDINGS 23: cooling_boost_kappa raising dMdt is NOT a contrasting "wrong sign" -- it moves the other factor of Eq 47, (C/6e-7 cgs)^(2/7), which rises with conduction by construction. Both knobs move Eq 47 correctly, in different variables.] L_leak is deliberately NOT scaled (leakage is bulk escape, not interface radiation). Requires f_A > 0; values < 1 are untested suppression territory. Default 1.0 = byte-identical. Single-knob use intended: combining with cooling_boost_mode != none or cooling_boost_kappa != 1 warns at load (double-boost / cross-knob).', category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True, validator=_validate_cooling_boost_fA),
    ParamSpec(name='phase1a_segFrac', default='0.1', info='Phase-1a integration segments as a fraction of the bubble age: dt_segment = phase1a_segFrac*(t_now-tSF). The driving terms (bubble structure, shell, P_HII) are frozen per segment, so this holds their staleness at a constant fraction of the expansion time at every object scale. Set to 0 to fall back to the fixed 30-yr SEGMENT_DURATION, which is a small step for a GMC but spans the entire free-streaming->Weaver relaxation of a compact HII region (docs/dev/phase1a-init/FINDINGS.md). Must be in [0, 1]: negative values would hit the same <=0 fallback as 0 and silently run stock segments, and above 1 a single frozen segment spans more than the bubble\'s whole age. Converged at the 0.03-0.3 measured range (0.1 -> 0.03 moves R2 at the compact-probe observed age by 0.11%).', category='input_solver', unit=None, exclude_from_snapshot=True, run_const=True, validator=_validate_phase1a_segFrac),
    ...
    ParamSpec(name='chi_e', default=1.2, info='Electron-per-hydrogen-nucleus factor n_e/n_H = 1 + Z_He*x_He for the HOT bubble (doubly-ionised He). Derived at load from x_He, Z_He. Multiplies n_H^2 in the bubble CIE cooling.', category='derived_init', unit='dimensionless', exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='mu_ion_shell', default=0.0, info='Ionised mean mass per particle for the ~1e4 K shell / HII region (singly-ionised He): mu_H/(2 + x_He*(1+Z_He_shell)). Derived at load from x_He, Z_He_shell. Used for the HII gas pressure and the shell structure (the hot bubble uses mu_ion instead).', category='derived_init', unit='m_H', exclude_from_snapshot=True, run_const=True),
    ParamSpec(name='chi_e_shell', default=1.1, info='Electron-per-hydrogen-nucleus factor n_e/n_H = 1 + Z_He_shell*x_He for the ~1e4 K shell / HII region (singly-ionised He). Derived at load from x_He, Z_He_shell. Multiplies n_H^2 in shell recombination and the Stroemgren balance.', category='derived_init', unit='dimensionless', exclude_from_snapshot=True, run_const=True),
    ...
    ParamSpec(name='Pb', default=0, info='Bubble pressure. ENERGY / IMPLICIT / TRANSITION: the THERMAL pressure of the shocked mechanical ejecta filling R1<r<R2, Pb = (gamma-1)*Eb/[(4pi/3)(R2^3-R1^3)] (bubble_E2P). Eb is fed by Lmech_total = Lmech_W + Lmech_SN, so Pb is wind-only ONLY in an SN-free run; it contains NO radiation pressure (F_rad is a separate additive force in the EOM and never enters Eb) and NO photoionised-gas pressure (P_HII is a separate field combined by max(), not added). ⚠ MOMENTUM PHASE: this field is OVERWRITTEN with the wind+SN ram pressure pRam(R2,Lmech_total,v_mech_total) (run_momentum_phase.py) so that the shell inner-edge density Pb/(k_B*T_ion) stays meaningful — it is NOT a thermal pressure there, and get_phii_c3c reads it as the confining pressure. Always gate on current_phase before comparing Pb across a run.', category='runtime_bubble', unit='Msun/Myr**2/pc'),
    ...
    ParamSpec(name='F_ram', default=0, info='⚠ MISNOMER, kept for schema stability: this is 4*pi*R2^2 * Pb, i.e. the force exerted on the shell by whatever Pb currently holds. In the ENERGY / IMPLICIT / TRANSITION phases that is the hot bubble THERMAL pressure force and is NOT a ram pressure; only in the MOMENTUM phase (where Pb aliases pRam) is it the true ram force pdot_total. Consequence: F_ram != F_ram_wind + F_ram_SN outside the momentum phase, and F_ram must be gated on current_phase before it is integrated or compared with F_HII / F_rad.', category='runtime_force', unit='Msun*pc/Myr**2'),
    ParamSpec(name='F_ram_wind', default=0, info='Wind momentum INJECTION rate feedback.pdot_W read straight from the SPS interpolators (= Mdot_w*v_w). It is written in every phase but equals a force on the shell ONLY in the momentum phase, where P_drive = P_HII + P_ram; in the energy / implicit / transition phases the wind acts through Pb instead and this quantity appears nowhere in the equation of motion. Do not use it as the wind share of a force budget outside the momentum phase.', category='runtime_force', unit='Msun*pc/Myr**2'),
    ParamSpec(name='F_ram_SN', default=0, info='SN momentum INJECTION rate feedback.pdot_SN read straight from the SPS interpolators. Same caveat as F_ram_wind: a shell force only in the momentum phase; in the energy era SN energy enters the dynamics through Eb (and hence Pb), not through this rate.', category='runtime_force', unit='Msun*pc/Myr**2'),
    ...
    ParamSpec(name='n_IF_Str', default=0.0, info='Stroemgren ionization balance density, aa61074-26.tex eq:nIF_Str. NOT capped: the min against shell_n0 was removed 2026-09-06 (PLAN.md W60) -- it bound on every row, because an rms over a profile that rises outward is >= its inner value by construction. Gates whether P_HII is computed at all, but since 2026-08-14 (C3c) no longer sets its value', category='runtime_shell', unit='1/pc**3'),
    ParamSpec(name='n_IF_Str_raw', default=0.0, info='Historically the pre-cap n_IF_Str; identical to n_IF_Str since the cap was removed 2026-09-06 (PLAN.md W60). Diagnostic only, never used in dynamics. See docs/dev/phii-identity/.', category='runtime_shell', unit='1/pc**3'),
