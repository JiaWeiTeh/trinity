...
Sibling to ``trinity.sps.sps_columns.CanonicalSpec`` (same declarative,
frozen-dataclass + module-level tuple pattern).  Consumed by:

* ``trinity._input.registry`` derivation helpers (``run_const_keys``,
  ``metadata_exclude_keys`` — Phase 5 drop-in replacements for the
  hand-curated lists in ``trinity._output.run_constants``).
* ``trinity._input.read_param`` (``validator`` wired in Step 5 / Phase 6;
...
class ParamSpec:
    ...
    # Snapshot/metadata routing — ``trinity._output.run_constants`` derives
