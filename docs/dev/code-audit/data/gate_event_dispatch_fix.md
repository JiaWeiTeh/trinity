...
### `maxr.jsonl` vs `outputs/probe_iscollapse_maxr/dictionary.jsonl`

ends via large_radius while EXPANDING - the case the fix targets

- rows: 155 (unchanged)
- physics columns differing: **0**
- `isCollapse` rows differing: **1**
    - row 154: True -> False at v2 = +21.5499 pc/Myr (EXPANDING - correct to clear)

**PASS**

### `momentum.jsonl` vs `outputs/phase6_momentum/dictionary.jsonl`

ends via stop_t, reaches phase 2 - no terminal event fires

- rows: 195 (unchanged)
- physics columns differing: **0**
- `isCollapse` rows differing: **0**

**PASS**

### `diffcfg_solo.jsonl` vs `outputs/phase6_cfgB/dictionary.jsonl`
