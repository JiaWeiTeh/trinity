...
690 findings, 16 S1 after revision. **The four
...
> ⚠️ **The findings-only invariant ended deliberately at commit `b05725b`.** Through
> that commit `git diff origin/main HEAD -- trinity/ test/ run.py param/` was empty,
> verified at every commit. On maintainer instruction (2026-08-08) the **fixing stage
> now shares this branch**, so that diff is *expected* to be non-empty from the next
> commit onward. This is not a regression — do not "restore" it. `PLAN.md` prefers
> fixes on a separate branch; the maintainer chose otherwise here. Fixes landing:
> `NUM-02` + `S11-R-02` (event dispatch / `isCollapse`), `S12b-R-01` (sweep run-name
> collisions).
