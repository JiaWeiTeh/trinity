...
5. **`dense_output` ≡ `t_eval`** (no benefit). **`odeint(mxstep=50k)` is the only free option for the Python `ODEintWarning("Excess work")`** — bit-identical (rel_n = 0) at 0.98–1.00× — and is **applied**: `trinity/shell_structure/shell_structure.py` passes `mxstep=_SHELL_ODE_MXSTEP` (=50000) on both odeint calls. **Note (2026-06-22):** this silences only the *excess-work* warning; the LSODA Fortran `t+h=t` **flood** is a separate ceiling, fixed by the `_NSHELL_MAX` clip guard (`get_shellODE.py:32,100`, `b27cede`) — see `OVERFLOW_FIX_PLAN.md`. Do **not** read line 5 as "mxstep fixes the flood".
...
### Net recommendation
The 100-sample redo **strengthens** the original call: do **not** restructure to `solve_ivp` for speed — its only advantage (the front-event trick) is narrower than first thought (energy-phase-only) and a net loss over a realistic run. The Python excess-work warning is silenced for free by the `mxstep` bump (done); the separate LSODA `t+h=t` flood is fixed by the `_NSHELL_MAX` clip guard (`b27cede`). Any `solve_ivp` migration is justified by **robustness/cleanliness** (explicit success flag, no uninitialised-tail consumption), not performance.
