# docs/dev/betadelta/diagnostics — betadelta hybr-solver diagnostics & plots (glossary)
...
`docs/dev/betadelta/PHASE2_ARMS.md`, `docs/dev/betadelta/stalling-energy-phase.md`,
`docs/dev/betadelta/HYBR_PLAN.md`. Companion harness dir: `docs/dev/betadelta/velstruct/`.
...
| `hunt_*` (h1–h6) | 6.0 | **hunt**: velocity-contamination sweep, 6 configs | `docs/dev/betadelta/velstruct/hunt.py` → `docs/dev/data/hunt_*.csv` | `plot_hunt.py` → `hunt_*` |
...
(`*_smoke` = a tiny `stop_t` smoke test). The hunt params live in `docs/dev/betadelta/velstruct/`.
...
**Velocity-contamination hunt (Phase 6.0, `docs/dev/betadelta/velstruct/plot_hunt.py`):**
...
- **Tracking note:** this README is the live tracker for the plotting/animation work so it
  does not collide with the canonical docs (`docs/dev/betadelta/stalling-energy-phase.md`,
  `docs/dev/betadelta/HYBR_PLAN.md`) that are being edited for Phase 6.1.
