...
## Harness
`docs/dev/scratch/transition-trigger/harvest.py <run_dir> [--csv out]` — reads a finished run,
...
`docs/dev/data/transition_mock_hybr.csv`; config: `docs/dev/scratch/transition-trigger/mock_hybr.param`.
...
`docs/dev/scratch/transition-trigger/dense_flat.param`.
...
`docs/dev/scratch/transition-trigger/steep.param`. rCloud = 23.4 pc.
...
`docs/dev/scratch/transition-trigger/steep_long.param`. Resolves both open G0 questions:
...
1–2 skipped — F2 eliminated at G0). Script: `docs/dev/scratch/transition-trigger/psens.py`.
