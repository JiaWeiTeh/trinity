...
| **→ NEXT: pin the single f_mix** | 4 fires the band but dense cores fire-then-recollapse; 2 is gentle but compact-only — needs the **recollapse physics call** (§8a, now emergent) ± a finer f∈(2,4] bracket + diffuse stop_t=8 spot-check; dense-edge (nCore 1e6) NaN stiffness ticket | ⏳ **the decision point** | `FINDINGS.md §10` open items |
