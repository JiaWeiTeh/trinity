...
| the physical in-ODE successor (f_A source term) | screen (P0) + all-9 offline coverage/edge map (P1: θ≈1 edge prediction FALSIFIED safe — no dMdt≤0 edge even at f_A=512) + **Phase 2 RUN 2026-07-06**: `cooling_boost_fA` wired into production (2 edit sites in bubble_luminosity.py + registry ParamSpec/validator), gated default-1.0 byte-identical, new `test_fA_source_boost.py` 9 tests, full pytest 742 green. **Phase 3 RUN 2026-07-06**: all 4 gates pass — LITERAL byte-identity at default (pre==postA==postB, A/A bit-identical), screen reproduces §2, live smoke fA=8 shows dMdt↓/θ↑ 29/29 (first live El-Badry sign). Remaining: 81-arm theta5s HPC matrix (first maintainer-gated phase — sbatch), bench5 Lancaster calibration, decision tree | 🟢 Phases 0–3 done; 4–6 open (Phase 4 = HPC, maintainer-gated) | `SOURCE_TERM_DESIGN.md §3` (start here), FINDINGS §15/§15a/§15b/§15c/§16 |
