...
| **El-Badry θ_target (current direction)** | calculator + mechanism verified + n-mapping resolved + λδv≈3 calibrated + **spec written** (`THETA_ELBADRY_SPEC.md`) | ⏳ **NEXT = IMPLEMENT** the gated `theta_elbadry` mode per the spec (3 registry params + 1 `effective_Lloss` branch), then validate on 8 configs ≥5 Myr | `THETA_ELBADRY_SPEC.md`, `ELBADRY_REFERENCE.md` §7–§9, `LANCASTER_REFERENCE.md` §7 |
