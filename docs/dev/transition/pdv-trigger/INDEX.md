...
| the physical in-ODE successor (f_A source term) | screen RUN 2026-07-06 (dial + El-Badry dMdt sign + stability); **Phase 1 RUN 2026-07-06**: all-9 offline coverage + edge map — the θ≈1 condensation-edge prediction FALSIFIED in the safe direction (no dMdt≤0 edge even at f_A=512; the source knob can't reach the f_κ crash); both controls stay unfireable. Remaining: Phase 2 wiring, Phase 3 gates, 81-arm theta5s (grid to 32 + bracket rule, per-class acceptance), bench5 Lancaster calibration, decision tree | 🟢 Phases 0–1 done; 2–6 open, fully spec'd | `SOURCE_TERM_DESIGN.md §3` (start here), FINDINGS §15/§15a/§16 |
