...
| the physical in-ODE successor (f_A source term) | screen (P0) + all-9 offline coverage/edge map (P1: θ≈1 edge prediction FALSIFIED safe — no dMdt≤0 edge even at f_A=512) + **Phase 2 RUN 2026-07-06**: `cooling_boost_fA` wired into production (2 edit sites in bubble_luminosity.py + registry ParamSpec/validator), gated default-1.0 byte-identical, new `test_fA_source_boost.py` 9 tests, full pytest 742 green. Phase 3 all 4 gates pass (LITERAL byte-identity, live El-Badry sign 29/29). **Phase 4 TOOLING READY 2026-07-06** (81 committed params + sbatch@6h + sync + analysis + dMdt reducer, all locally validated) — 🟡 **awaiting maintainer HPC submission** (`./sync_theta5s.sh submit`; the matrix is NOT yet run). **Phase 4 in-container COMPLETE 81/81 2026-07-11 (§15e, PROVISIONAL — not HPC): collapse-law p=3.330 CONFIRMS registered p_source≈3.3; both controls never fire; 3 classes (normal_n1e3 fires unmodified / 6 configs need f_A f_fire 4–12 / 2 controls). ASSUMED — must still be re-run on HPC (`run_theta5s.sbatch`) and all downstream re-checked.** Next-chat handoff written in `SOURCE_TERM_DESIGN.md`. | 🟢 Phases 0–3 done; Phase 4 in-container 81/81 done (⚠️ PROVISIONAL §15e, HPC confirmation pending); **Phase 5 ✅ pre-step ✅ + in-container run COMPLETE 2026-07-12 (§15g pre-step: L21b Table-1 [V]-imprinted `LANCASTER_REFERENCE §7b`, "M_*=5000" falsified→ε_* ratios, 60 params frozen; §15h run 60/60 ran, 59 compliant (1 dense diag wall-killed): FIRE MAP threshold 1→4→12→>16→>16 as n̄ falls; Θ_cum L21b calibration (diagnostic arms, all complete) — bench3 enters band [0.90,0.99] at f_A≈16, bench2/bench1 need >16/≫16, dex-vs-El-Badry never below 0.85 → **no single global f_A reproduces L21b across density; required boost climbs steeply toward low density = route-a boundary**. PROVISIONAL (in-container, HPC re-confirm before paper). ONLY HPC need = theta5s §15e confirmation → repo-root `temporary-HPC-runs.md`); **Phase 6 (ship decision) = next open item — ALL DECISION DATA IN HAND (`FINDINGS §15j`, 2026-07-19)**: theta5s HPC-CONFIRMED (p=3.330 exact); bench5 HPC-CONFIRMED (FIDELITY OK, zero fire flips); bench6 measured — f_A band-entry 13.9/53.5/74.8 (all clean benches reach the band; f_A(n̄)≈315·n̄^−0.335, spread 5.39×; thresholds 1→4→12→24→64) while **f_mix is eliminated as a CALIBRATION knob** (never reaches the band ≤8, wrong-sign dose-response, fm8 false-fires). Tree row 3 verdict. **Phase-6 ruling STARTED 2026-07-22 (`FA_STATE_COUPLED.md §3`): f_mix RETAINED as opt-in fallback, staged retirement gated on the state-coupled f_A; default stays `none`.** sole remaining HPC item = dMdt reducer on Helix raw arms. ⛔ **2026-07-27 external review (`FINDINGS §17`): the fm "elimination" was a METRIC ARTIFACT** (bench6 fm Θ_cum omitted the boost). ✅ **X1–X4 EXECUTED 2026-07-28 on `feature/pdv-trigger-5` (`FINDINGS §18`/`§19`/`§20`):** metric fixed and CSVs regenerated (f_A side bit-stable, gate ≤1e-9 → measured 2.6e-16); corrected fm band entry bench3 ≈4 MEASURED, bench2/bench1 >8 (extrapolated ≈8.2/11.9) — **uniformity spread 2.96× vs f_A's 5.39×, so the head-to-head INVERTS** as an estimate the fm≤8 grid cannot settle; "f_mix eliminated" WITHDRAWN everywhere; the §16 double-boost BOUNDED OUT of the fire map (§19); wind-only cap + metric-2 slope closed (§20). New: a large frozen-no-root share in Θ_cum on BOTH knobs, worse on f_A (`data/bench_stale_segments.csv`). f_A-side numbers + SC-0 unaffected. **Next open work = the maintainer re-presentation of clause 1 (`SOURCE_TERM_DESIGN §3`), then optionally fm≈{12,16} on bench1/bench2 to MEASURE the inversion** | `SOURCE_TERM_DESIGN.md` handoff + §3, FINDINGS §15/§15a–§15i/§16/§17, `LANCASTER_REFERENCE §7b`, `data/bench5_{analysis,elbadry_prediction}.csv`, `runs/data/bench5_summary.csv` |
| **state-coupled f_A (successor workstream, PLANNED 2026-07-22)** | replace the scalar with f_A_state = L_int^EB(R2, Pb; λδv)/(L2+L3)_prev — derive the measured f_A(n̄) instead of fitting it; ONE free physical constant (λδv≈3–3.5 target) | **`FA_STATE_COUPLED.md` = THE single plan** (SOURCE_TERM_DESIGN §4 sketch promoted there). **🔴 TERMINAL 2026-07-25 — SC-0 FAILED (§15k, `fa_state_screen.png`): C1 misses by 3.3×/4.5× per criterion, C2 by 2–8 dex (ℓ_cool unreachable for all p<1), C3 by 56× — none holds one constant to the pre-agreed factor ~2. Per SC-0's own FAIL clause the ladder STOPS: SC-1…SC-5 not to be started, no production code written, the fitted f_A(n̄) stays a measurement of record and never a shipped model, and the f_mix retirement ladder halts permanently at R0 (done — registry `info` lines, help-text only). Reopening needs a NEW candidate law, entering at SC-0.** | `FA_STATE_COUPLED.md`, `ELBADRY_REFERENCE.md §7/§9`, `FINDINGS §15j` |
...
*Index rewritten 2026-07-01 during the pt2⇄pt3 reconciliation; §3 f_A row updated 2026-07-27
(external review `FINDINGS §17` — fm-elimination correction, X-plan). Update §2/§3/§5 whenever a
doc, step, or branch changes.*
