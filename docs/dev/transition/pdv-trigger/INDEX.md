...
| **El-Badry θ_target (NEW direction)** | feed θ(λδv,n) into gated `theta_target`; offline calculator + θ_target-mechanism verified + n-mapping resolved | ⏳ **NEXT** — only λδv calibration left, then wire a gated `theta_elbadry` mode (θ(λδv, n_amb(R2)) + θ_max ceiling), byte-identical-off, run ≥5 Myr | `ELBADRY_REFERENCE.md` §7–§9, `data/make_elbadry_theta.py`, `data/make_nmap_verify.py` |
