...
| **state-coupled f_A (successor workstream, PLANNED 2026-07-22)** | replace the scalar with f_A_state = L_int^EB(R2, Pb; λδv)/(L2+L3)_prev — derive the measured f_A(n̄) instead of fitting it; ONE free physical constant (λδv≈3–3.5 target) | **`FA_STATE_COUPLED.md` = THE single plan** (SOURCE_TERM_DESIGN §4 sketch promoted there). SC-0 offline falsification screen OPEN (read-only, pre-ruling); SC-1+ gated on the Phase-6 ruling; SC-4 L21b acceptance gate = the pre-agreed default-flip bar | `FA_STATE_COUPLED.md`, `ELBADRY_REFERENCE.md §7/§9`, `FINDINGS §15j` |
