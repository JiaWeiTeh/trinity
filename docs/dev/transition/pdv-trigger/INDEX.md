...
| the physical in-ODE successor (f_A source term) | screen RUN 2026-07-06 (dial + El-Badry dMdt sign + stability); direction endorsed; plan consolidated to ONE doc after a 2-agent audit (config coverage + literature benchmarks): Phases 1–6 open — offline completeness (incl. small_1e6/normal_n1e3 fixtures + edge map), wiring, gates, 81-arm theta5s (grid to 32 + bracket rule, per-class acceptance incl. the two controls), **bench5 Lancaster-matched calibration** (Θ_cum ∈ [0.9,0.99], 1−θ tracks ≤0.5 dex), decision tree | 🟢 Phase 0 done; 1–6 open, fully spec'd | `SOURCE_TERM_DESIGN.md §3` (start here), FINDINGS §15/§16 |
