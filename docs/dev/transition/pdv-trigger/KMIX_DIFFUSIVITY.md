...
| `f_κ(n)=[θ_target/θ_0]^(1/q)=f_mix^(1/q)`; ≈4 compact, ≈60 diffuse; "analytic skeleton of the calibration" | this is exactly our composed form (§0–§8); the anchors match `kappa_blowout_calibration` | ✓ **match** |
...
| f_κ≈60 is not a conduction coefficient → faithful κ_mix is the honest object | matches §13 | ✓ **match** |
