...
| 41 | **f_A Phase 4 PROVISIONAL in-container fallback** — theta5s matrix run in Claude's ephemeral container (maintainer had no HPC access), restart-limited & fast-arm-biased; only the fastest-firing arms complete (fail_repro control never fires ✓, small_dense fires fA≥4, normal_n1e3 fA12/16; baselines + diffuse configs wall-kill). **ASSUMED, not authoritative — re-run on HPC (row 40) and re-check ALL downstream** | `FINDINGS.md` §15e, `SOURCE_TERM_DESIGN.md` §3 Phase 4 | `runs/params/theta5s/` (81) | `python runs/run_theta5s_local.py --out $WS/t5s_out --workers 3 --per-arm-timeout 1800 --summary runs/data/theta5s_summary.csv`; merge each container's arms via `python runs/checkpoint_theta5s.py --out $WS/t5s_out --summary runs/data/theta5s_summary.csv` | `runs/{run_theta5s_local.py,checkpoint_theta5s.py}`, `runs/data/theta5s_summary.csv` (carries PROVISIONAL header) | 🟡 in-container partial, HPC pending |
