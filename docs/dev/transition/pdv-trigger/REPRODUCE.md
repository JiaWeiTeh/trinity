...
| 48 | **K1–K4 — the f_κ re-open campaign (102 arms): PLANNED, NOT RUN** — K1 the missing third leg of the band-entry head-to-head (bench1/2/3 × f_κ ∈ {2,3,4,6,8,12,16,24,32} × prod/diag), K1b dense fire-map completeness, K2 the condensation-squeeze fine grid, K3 fate determinism, K4 the f_mix fm{12,16} closure. Predictions P1–P5 and gates G0–G5 are pre-registered. **Blocked on the §6.0 maintainer grid ruling; the param generator is deliberately not written yet.** | `KAPPA_REOPEN_PLAN.md` §3–§6 | `runs/params/bench7/` (to be generated) | `python runs/make_kappa_reopen_params.py` (not yet written) → `sync_bench.sh bench7` | `runs/data/bench7_summary.csv` + `data/make_bench7_analysis.py` (both pending) | 🔴 not run |
