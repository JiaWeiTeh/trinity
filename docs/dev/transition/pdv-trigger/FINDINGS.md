...
## 15h. [data — IN-CONTAINER, IN PROGRESS] Phase 5 bench5 campaign — 20/60 done, running to 60 in-container (2026-07-12)

> ⚠️ **IN-CONTAINER, IN PROGRESS (not HPC — and per the maintainer, does NOT need HPC).** As of the
> latest tick **20/60 arms are compliant** (all production so far). **Maintainer directive 2026-07-12:
...
to t=0. **20/60 completed** (all production; vs the prior lost pt4 session's 1–3/60 over 8 h — the
...
**FIRE MAP (production arms; FIRE = actually fired cooling_balance, from `data/bench5_analysis.csv`; updated 20/60):**
...
| bench2 (m1e5, 10pc) | 690 | 0.986 | 6,8,12,16 (fa4 running) | **>16** (NOFIRE ≤16) | fa6/8/12/16: 0.55,0.58,0.66,0.69 | all NOFIRE→energy-driven to t=5 |
...
**Read (20/60 in-container):** a clean, steeply-rising **density gradient in the f_A fire threshold** —
...
registered El-Badry θ_EB falling with density (`§15g`; θ_EB 0.999→0.995→0.986→0.948). **The two diffuse
...
(20 arms) + `runs/data/bench5_traj/` (20 trajectories), `data/bench5_elbadry_prediction.csv` (registered,
