...
`FINDINGS.md §15h`: 50/60 arms compliant so far (all production; 12 FIRED + 6 NOFIRE (bench3 fa4/6/8 + bench2 fa8/12/16)). FIRE MAP: bench5(n̄=2.28e5) + bench4(n̄=4.42e4) fire at every f_A≥4; bench3(n̄=5520) at f_A≥12 (NOFIRE≤8);
...
campaign RUNNING (`§15h`) → 50/60 done (all prod; 12 FIRED + 6 NOFIRE bench3+bench2). Maintainer ruled ALL 60
...
### Phase 5 🟡 — literature calibration: matched-config benchmarks (pre-step ✅ + 60 params frozen `§15g`; in-container run PARTIAL 50/60 fire map `§15h` 2026-07-12; Θ_cum calibration HPC-deferred)
