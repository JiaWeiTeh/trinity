...
| `runs/params/bench7/` (118 `.param` files, built 07-29, `runs/make_kappa_reopen_params.py`) | E8 | kappa 2–32 + fmix 2–16 | ✅ CLEAN as **inputs**; carries **no data at all**. K1–K4 are **NOT RUN** as of 2026-07-29 — there is no `bench7_summary.csv`, no `bench7_traj/`, no `bench7_hashes.csv`. ⚠️ The K4 phase (24 f_mix arms) rests on a **read, not confirmed**, §6.0(c) ruling; check `F_MIX_K4` before submitting. Pinned by `test/test_bench7_params.py`. **↻ 2026-08-03: the "NOT RUN" clause is spent — all 174 arms ran 2026-07-30 and the three harvest files exist; grade the *data* from the `runs/data/` rows below, this row now grades the inputs only.** |
| `farea_screen.csv` + `farea_screen.png` (built 08-03, `data/make_farea_screen.py`) | E8 | kappa / fA / **both at equal doses**, f ∈ {1,2,4,8,16} | ✅ CLEAN as a **frozen-state screen**, and ⛔ **FLAG-(a) by construction**: 30 single-call production solves at the two committed captured states (`test/data/dR2_stiff_state_fixture.json`, `residual_resample_fixture.json`), which are EARLY snapshots (θ_snapshot ≈ 0.009 / 0.001). Scope is fixed by `kappa-3way/FINDINGS §12a`: **falsifier and sign-check only — no number here is a full-run calibration**, the P1 precedent being that frozen exponents missed the integrated ones by ~2×. The one principled exception is the layer-invariance identity (`F_AREA_PLAN §2.2`), which is itself a per-call statement and is the thing this screen refutes. Quotable: the Ṁ exponent q = 0.279–0.285 **at these states**, the f^{5/7} anchor motion, the A0.1–A0.5 verdicts, and the "no viability ceiling to f = 16" observation. ⛔ Never quote its L-ratios or θ_snapshot as band-entry or calibration numbers. |
