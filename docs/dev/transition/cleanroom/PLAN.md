...
**About this document**
- **Status (2026-06-20):** 🟢 **C0 SUBSTRATE CERTIFYING + 6/6 PHYSICS RESULT IN.** All 6 full `stop_t=6` runs complete. C0.3 ✅; `res_T0_struct` ✅ span-wide; `res_beta` = FD truncation (refinement directionally confirmed, genuine 4× finishing); C0.1 (Weaver null) next (cores free). **No substrate defect.** **Physics (6/6):** stall 6/6; `f_ret` plateaus 0.25–0.40, **NONE reach the observed 0.01–0.1 band** (unanimous under-cooling); steep `pl2_steep` blows out (R2 ~14× rCloud, profile-dependent fate); negative β in 6/6 at the SN surge. **Verdict: the stall is primarily a physics-completeness signal (under-cooling: missing mixing-layer physics) + a profile-dependent fate — NOT a tunable-threshold bug.** Figures: `figures/fret_verdict`, `beta_repressurization`, `cert_residuals`. No production change.
