...
**About this document**
- **Status (verified 2026-06-16):** ✅ **SHIPPED** (verified 2026-06-16) — complete offline results record; kept with the plan until P-shadow ships.
...
## Figures (P0/P-sens) — reproducible from the committed CSVs [2026-06-17]
Generated **offline from `docs/dev/data/transition_*.csv`** (no sim re-run) by
`docs/dev/transition/harness/make_p0_figures.py`; every firing epoch is re-derived
from the CSV columns, so the figures self-check against the numbers above (printed
on run; they reproduce the committed summary CSV exactly — dense-flat
F0=F1=Eb-peak@0.197 with retained-at-end 0.29; steep blowout@2.728; steep
retained-at-end **0.35 (1 Myr run) / 0.44 (4 Myr run)** — the "≈0.35" elsewhere is
retention *at blowout*, a different epoch). The WR surge is data-derived as the
first β+δ<0 at **3.178 Myr**; P0.md's earlier "3.078" marker is stale (β+δ=+1.47
there), but blowout (2.728) precedes both so the pre-surge conclusion is unchanged.
**Independently verified 2026-06-17** (two subagents: byte-for-byte data match +
line-by-line script audit, both pass). LaTeX-free: the script sets
`text.usetex=False` (the repo `paper/_lib/trinity.mplstyle` forces usetex, which
has no TeX in this container).

Reproduce: `python docs/dev/transition/harness/make_p0_figures.py`
→ writes `docs/dev/transition/figures/`:

| figure | shows |
|---|---|
| `p0_divergence_map.png` | firing epoch of F0/F1/F2/F4/Eb-peak for all 5 configs (log-t) — candidates do not coincide; flat→cooling, steep→blowout |
| `p0_overlay_dense_flat.png` | clean cooling transition: F0 = F1(η0.3) = Eb-peak @0.197 |
| `p0_overlay_steep_long.png` | the crux: R2>rCloud blowout @2.728, before the WR surge; ratio_F0 never < 0.05 |
| `p0_reset_pathology.png` | across the surge F0 spikes 0.44→0.67 / F1 retreats 0.65→0.56 — neither cooling form rescues the steep stall |
| `p0_divergence_map.csv` | the summary table (config × each candidate epoch × retained fraction) behind the divergence-map figure |
...
8. ~~**Overlay figures** per config (Eb(t)/ratio(t)/frac_cum with firing epochs).~~
   **DONE [2026-06-17]** — 4 figures + summary CSV in `docs/dev/transition/figures/`
   (see the Figures section); reproducible offline via `make_p0_figures.py`.
9. ~~Finalize the divergence map → **Gate G0**.~~ **DONE** — G0 = PROCEED (above).
