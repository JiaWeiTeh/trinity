...
| 5 | `Pb ≡ P_HII` to machine precision (pressure continuity by construction, with P_ram=0, F_ISM=0) | §4 | `trinity/phase1b_energy_implicit/run_energy_implicit_phase.py:439–467` — `P_HII` from Strömgren balance; `P_drive = max(Pb, P_HII)`; `P_ram=0.0` (line 467: comment "no ram pressure in implicit phase") | ✅ | ⚠️ verified true 2026-06-22; **no longer true of `main`** — C3c (`c43a50e`, 2026-08-14) took `P_HII` off the Strömgren cap, so it is `0.0` on every implicit row and the identity is gone |
