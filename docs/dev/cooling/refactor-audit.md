...
> renamed and `src/` deleted), the config layer was rebuilt into a
> `registry.py`/resolver system (so `read_param.py` line/mechanism refs are
> obsolete), and the SPS `_get_legacy_sb99_filename` helper this plan says to
> mirror **never existed** — the SPS refactor landed instead as a resolver
> (`_resolve_sps_bundle` in `trinity/_input/registry.py`) that resolves to a
> single hardcoded default file keyed on `SB99_rotation`/`ZCloud`. References
> below to mirroring `_get_legacy_sb99_filename` should be read as "mirror the
> SPS resolver pattern."

**About this document**
- **Status (updated 2026-06-22):** 🟡 **PARTIAL** — the core loader decouple (PR-1–PR-4) has **not** shipped (the cooling loaders are still hardcoded as described), but two side items have since landed: the `T<1e4` non-CIE floor → table-derived floor (§4 #9; `net_coolingcurve.py:130–131`, `cc8ae76`; the `magic-numbers/` T-floor story) and the silent `NameError`→explicit `ValueError` for an unsupported `ZCloud` (§4 #1; `read_cloudy.py:298–302`, `3deec3d`). Cited line numbers have drifted ~5–45 lines.
...
- The cooling module is **structurally cleaner than pre-refactor SPS** — every
  ...
  (`trinity/cooling/net_coolingcurve.py:22`), and physics phases only touch
...
- The **loader-side coupling is the SB99-loader problem twice over**:
  - **Non-CIE** (`trinity/cooling/non_CIE/read_cloudy.py:266-336`, `get_filename`)
    hardcodes the
  ...
  - **CIE** (`trinity/_input/read_param.py:397-414`, the inline ZCloud branch in
    Step 7) uses an integer index
...
- **Four PRs**, each back-compat and independently revertable, mirroring the
  ...
  1. `path_cooling_nonCIE` becomes a real path knob; remove
     ...
     behind a legacy-fallback resolver (mirroring the SPS
     `_resolve_sps_bundle` pattern in `trinity/_input/registry.py`).
...
- **Headline risks:** (a) cube-cache staleness if a user swaps cooling
  ...
  saved at `:261`, is keyed only on stem, no axis hash); (b) silent physics-altering
...
- **Non-CIE** (`T < 10^5.5 K`): 3D table over `(ndens, T, phi)`.
  ...
  (`trinity/cooling/non_CIE/read_cloudy.py:22-134`), which:
  (a) consults `params['SB99_rotation']` and `params['ZCloud']` to build
  an OPIATE filename via `get_filename()`
  (`trinity/cooling/non_CIE/read_cloudy.py:266-336`); (b) reads the table
  via `astropy.io.ascii.read` keyed by hardcoded column names
  `ndens, temp, phi, cool, heat`
  (`trinity/cooling/non_CIE/read_cloudy.py:179-187`); (c) builds two 3D
  cubes plus a net interpolator
  (`trinity/cooling/non_CIE/read_cloudy.py:94-132`); (d) caches the rebuilt
  cubes as `<stem>_cube.npy` next to the source file
  (`trinity/cooling/non_CIE/read_cloudy.py:170-172, 261`).
- **CIE** (`T ≥ 10^5.5 K`): 1D `logΛ(logT)` table loaded once at startup
  by `trinity/main.py:162-171` via `np.loadtxt(path, unpack=True)` →
  `scipy.interp1d`. The resolved path is computed inline in Step 7 of
  `trinity/_input/read_param.py:397-414` (after the `resolve_all(params)`
  registry pass) from the integer-index selector
  `path_cooling_CIE ∈ {1, 2, 3}` (when `ZCloud == 1`) or pinned to a single
  Sutherland-Dopita file (when `ZCloud == 0.15`).

`net_coolingcurve.get_dudt(age, ndens, T, phi, params)`
(`trinity/cooling/net_coolingcurve.py:22-163`) reads the pre-built
structures off `params['cStruc_*']`, dispatches on T against the
hardcoded `10^5.5 K` boundary
(`trinity/cooling/net_coolingcurve.py:93, 95`), evaluates the appropriate
...
1b:
`trinity/phase1_energy/run_energy_phase.py:125-130`
(`COOLING_UPDATE_INTERVAL = 5e-2 Myr`,
`trinity/phase1_energy/run_energy_phase.py:56`) and
`trinity/phase1b_energy_implicit/run_energy_implicit_phase.py:560-561`
(`COOLING_UPDATE_INTERVAL = 5e-3 Myr`,
`trinity/phase1b_energy_implicit/run_energy_implicit_phase.py:108`).

## 2. Configuration surface

| Param | File:line | Default | Role |
|-------|-----------|---------|------|
| `path_cooling_CIE` | `trinity/_input/default.param:190` (spec `registry.py:336`) | `3` | Integer index into the hardcoded `{1:Cloudy, 2:Cloudy+grains, 3:Gnat-Ferland}` table (when `ZCloud == 1`). Resolved to a path inline in Step 7 of `read_param.py:402-414`. **Cannot today be a path string** — `int(...)` at `read_param.py:408` will `ValueError`. |
| `path_cooling_nonCIE` | `trinity/_input/default.param:193` (spec `registry.py:337`) | `def_dir` | Directory of OPIATE cubes; resolves to `lib/default/opiate/` via the `_resolve_path_cooling_nonCIE` resolver (`registry.py:192-200`). Used by `read_cloudy.get_filename()` to build per-age filenames. |
| `ZCloud` | `trinity/_input/default.param:62` (spec `registry.py:290`) | `1` | Drives CIE file selection AND non-CIE filename's `Z_str`. Whitelisted to `{1.0, 0.15}` in both spots. Also drives dust opacity / other metallicity-keyed physics — **not deprecated by this refactor**. |
| `SB99_rotation` | `trinity/_input/default.param:133` | `1` | Used by both the SPS loader AND `read_cloudy.get_filename()`'s `{rot|norot}` discriminator. The non-CIE coupling is the §5.1 hot spot. |
| `cool_alpha` / `cool_beta` / `cool_delta` | `trinity/_input/default.param:279-285` | `0.6 / 0.8 / -6/35` | Cooling-related physics constants for the analytic bubble model; **orthogonal to the table loaders** — flagged only so they aren't accidentally pulled in. |
...
downstream code needs these six `cStruc_*` entries — now declared as
`ParamSpec(...)` entries at `trinity/_input/registry.py:400-405`
(all marked `exclude_from_snapshot=True, metadata_exclude=True`) and
listed in `trinity/_input/dictionary.py:1171-1177`:
...
`get_dudt()` (`trinity/cooling/net_coolingcurve.py:62-67`) reads:
...
|---|-----------|-------|-----------|
| 1 | `trinity/cooling/non_CIE/read_cloudy.py:47, 59, 266-336` (`get_filename`) | Hardcoded OPIATE filename grammar; reads `SB99_rotation` and `ZCloud`; whitelists Z ∈ {1.0, 0.15}; if `ZCloud` is neither, `Z_str` is never bound and line 312 hits `NameError`. | PR-1: replace with `path_cooling_nonCIE` as a real path/folder knob; move `get_filename()` behind a legacy-fallback resolver, mirroring the SPS `_resolve_sps_bundle` resolver pattern in `trinity/_input/registry.py` (the SPS refactor never introduced a `_get_legacy_sb99_filename` helper). **Partly shipped (`3deec3d`):** the silent `NameError` on an unsupported `ZCloud` is now an explicit `ValueError` (`read_cloudy.py:298–302`); the path-knob decouple (PR-1 proper) is still pending. |
| 2 | `trinity/cooling/non_CIE/read_cloudy.py:182-187` | Hardcoded column names `ndens, temp, phi, cool, heat`. No header-vs-position dispatch; no per-column units / log/linear. | PR-2: `cool_col_nonCIE_<canonical>  <file_column>  <units>  <log\|linear>` declarations, paralleling `sps_col_<canonical>`. |
| 3 | `trinity/cooling/non_CIE/read_cloudy.py:189-194` | "Wrong sign" auto-flip on cooling/heating columns. Hides a real convention mismatch under a `print` warning. | PR-2: a per-column `<units>` declaration that includes a `cgs_neg` alias for the negative-cgs case. Make the convention explicit, not auto-detected. |
| 4 | `trinity/cooling/non_CIE/read_cloudy.py:200-210` (`create_limits`) + 227-233 (fill loop) | Axis ticks built by `np.log10(set(col))` then rounded to **3 decimal places**; the fill loop also rounds to 3 dp for the `np.where` lookup. Fragile to any OPIATE-incompatible grid spacing. Note: this exact rounding mismatch caused an `IndexError` previously, fixed in commit c1589fc. | PR-2: optional `<grid_decimals>` declaration per axis, defaulting to 3 (preserves OPIATE bit-exactness); document the requirement that the file is on a regular log-spaced grid. |
| 5 | `trinity/cooling/non_CIE/read_cloudy.py:170-172, 261` | Cube cache (`<stem>_cube.npy`) keyed only on the source filename's stem. Same path, different file contents → silently uses stale cache. | PR-2: include axis-array hash (`hashlib.md5` of the concatenated tick arrays) in the cache filename, OR check axis-array equality before reusing. |
| 6 | `trinity/cooling/non_CIE/read_cloudy.py:298-307` (age-discovery loop) + `trinity/cooling/non_CIE/read_cloudy.py:339-343` (`get_fileage`) | Age list discovered by `os.listdir` filtering for `.dat`, with file age parsed from a **fixed-width slice** `filename[idx+3:idx+11]` (`trinity/cooling/non_CIE/read_cloudy.py:343`). Brittle to any filename containing `age` outside the OPIATE convention. | PR-1: when `path_cooling_nonCIE` is a single-file path (user mode), skip age discovery entirely — there is no per-age cube set. When it's a folder, allow a configurable file-age regex. |
| 7 | `trinity/_input/read_param.py:397-414` (CIE-path resolution, inline in Step 7 after `resolve_all(params)`) | `path_cooling_CIE` is parsed as `int()` (line 408); user **cannot** point it at a custom file. If `ZCloud` is neither 1 nor 0.15 the if/elif silently falls through (no `else` raise), leaving `params['path_cooling_CIE'].value` as the integer 3 → `main.py:165` calls `np.loadtxt(3, ...)` → cryptic error. | PR-3: detect path-vs-int at line 408; if a string-looking value, use it verbatim; else apply the legacy index table. Raise an explicit `ValueError` instead of falling through. |
| 8 | `trinity/cooling/net_coolingcurve.py:93, 95` | Hardcoded `5.5` (in log10 K) as the CIE/non-CIE boundary. Currently hand-wired to the OPIATE table's `T_max` and a typical CIE-table `T_min`. | PR-2: derive from the loaded tables — `max(cStruc_cooling_nonCIE.temp)` and `min(cStruc_cooling_CIE_logT)` are already in scope; the magic literal can be replaced with a sanity assertion that the two ranges meet near 5.5. |
| 9 | `trinity/cooling/net_coolingcurve.py:85` | `if T < 1e4: T = 1e4` — floor hardcoded to the OPIATE non-CIE table's `T_min`. | PR-2: floor to `min(cStruc_cooling_nonCIE.temp)` instead of the constant. **✅ SHIPPED (`cc8ae76`):** now `net_coolingcurve.py:130–131` does `if log10(T) < nonCIE_Tmin: T = 10**nonCIE_Tmin` (table-derived floor). |
| 10 | `trinity/cooling/CIE/read_coolingcurve.py:25` (`get_Lambda(T, cooling_CIE_interpolation, metallicity)`) | `metallicity` arg accepted but never used in the body (`:60-65`). `net_coolingcurve.py:88, 148` pass `params_dict['ZCloud'].value` into it for no functional effect. | PR-4: drop the unused arg; remove the two `ZCloud` reads from `net_coolingcurve.py`. |
| 11 | `trinity/cooling/non_CIE/read_cloudy.py:130` | `netcooling = cool_cube - heat_cube` is in **linear** space, but the standalone `cooling_interpolation` and `heating_interpolation` interpolate in **log** space (`read_cloudy.py:94-97`). The two consumers don't see the same units. | PR-2: document the unit convention per `cStruc_*` entry in the canonical registry; flag whether each cube is log or linear in the `cool_col_nonCIE_*` declarations. |
...
params)` (`trinity/cooling/net_coolingcurve.py:22`). The only direct
loader-imports outside `trinity/cooling/` are at:

- `trinity/bubble_structure/bubble_luminosity.py:33` — imports
  `net_coolingcurve` only (no direct loader reach-through).
- `trinity/phase1_energy/run_energy_phase.py` — imports
...
`trinity/bubble_structure/bubble_luminosity.py:1074`. So every
...
### 5.1 `trinity/cooling/non_CIE/read_cloudy.py` — 🔴 loader (main refactor target)
...
### 5.2 `trinity/cooling/CIE/read_coolingcurve.py` — 🟡 (small)

`get_Lambda(T, cooling_CIE_interpolation, metallicity)`
(`trinity/cooling/CIE/read_coolingcurve.py:25`) takes a `metallicity` arg
but never uses it in the body (`:60-65`); the docstring's table-selection
note still hints at a would-be metallicity branch that was never wired in.
The actual table choice has already been baked into
...
`trinity/cooling/net_coolingcurve.py:88, 148` that pass it.

### 5.3 `trinity/_input/read_param.py` and `default.param` — 🔴 plumbing

CIE path resolution lives inline in Step 7 of `read_param.py:397-414`,
**after** the new `resolve_all(params)` registry pass (line 395) — it is
not a def-sentinel resolver because the integer-index preset is keyed on
`ZCloud`, so it stays inline rather than in `registry.py`:

- `path_cooling_CIE` parsed as `int()` (line 408) → user can't set it to
...
- Else: silent fall-through (no `else` raise); `params['path_cooling_CIE'].value`
  remains the integer 3 (default) and `main.py:165` later calls
  `np.loadtxt(3, ...)`.
...
Non-CIE path resolution now runs through the `_resolve_path_cooling_nonCIE`
resolver (`trinity/_input/registry.py:192-200`), invoked by `resolve_all`:
handles `def_dir` correctly; user-supplied folder paths are also handled.
The SB99/OPIATE coupling lives inside `read_cloudy.get_filename()`
downstream.

Runtime containers — the six `cStruc_*` entries are now `ParamSpec(...)`
declarations at `trinity/_input/registry.py:400-405` (listed in
`dictionary.py:1171-1177`); see §3.
...
### 5.4 `trinity/main.py:162-173` — 🟢 (one-line touch in PR-3)
...
### 5.5 `trinity/cooling/net_coolingcurve.py` — 🟡 (constants, no logic)

166 LOC. The only changes needed:

- `trinity/cooling/net_coolingcurve.py:85` — floor `T` against
  `min(cStruc_cooling_nonCIE.temp)` instead of literal `1e4`.
- `trinity/cooling/net_coolingcurve.py:93, 95` — derive the `5.5` cutoff
  from the loaded tables (or sanity-assert the two ranges meet there).
- `trinity/cooling/net_coolingcurve.py:88, 148` — drop the
  `params_dict['ZCloud'].value` argument when PR-4 lands.

No interpolator-call patterns change. The body that does the CIE/non-CIE
dispatch (`trinity/cooling/net_coolingcurve.py:102-158`) stays as-is.

### 5.6 Phase consumers — 🟢

- `trinity/phase1_energy/run_energy_phase.py:125-130` and
  `trinity/phase1b_energy_implicit/run_energy_implicit_phase.py:560-561`
...
- `trinity/bubble_structure/bubble_luminosity.py:1074` calls
  `net_coolingcurve.get_dudt(...)` — interface unchanged.

### 5.7 `trinity/_output/cloudy/snapshot_to_deck.py` and the plotting code — 🟢 (orthogonal)

No direct loader access; no `cStruc_*` reads. Unaffected. (Note: `src/_plots/`
no longer exists — plotting was split out into `paper/figures/` in the #636
package restructure; none of it touches the cooling loaders either.)

## 6. Per-consumer change matrix

| File | Touches filename? | Touches columns? | Touches interpolators / cubes? | Change needed |
|------|:--:|:--:|:--:|--|
| `trinity/cooling/non_CIE/read_cloudy.py` | ✅ | ✅ hardcoded names | ✅ builds them | 🔴 rewrite with column map; move `get_filename()` to legacy fallback |
| `trinity/cooling/CIE/read_coolingcurve.py` | — | — | — (reads interp only) | 🟡 PR-4 drop unused `metallicity` arg |
| `trinity/main.py:162-173` | — | ✅ positional 2-col | builds CIE interp | 🟡 PR-3 swap `np.loadtxt` for `load_cie_table` helper |
| `trinity/_input/read_param.py:397-414` (CIE) + `registry.py:192-200` (non-CIE) | resolves both | — | declares containers (in `registry.py`) | 🔴 add path-vs-int dispatch for CIE; add column-map plumbing; relocate OPIATE grammar |
| `trinity/_input/default.param` | declares legacy params | — | — | 🔴 add `cool_col_cie_*` / `cool_col_nonCIE_*` blocks; keep `path_cooling_CIE  3` and OPIATE-style `path_cooling_nonCIE  def_dir` as permanent fallbacks |
| `trinity/cooling/net_coolingcurve.py` | — | — | reads them | 🟡 replace literals (`1e4`, `5.5`) with derived limits; PR-4 drops unused `ZCloud` arg |
| `trinity/phase1_energy/run_energy_phase.py` | — | — | ✅ refreshes cube | 🟢 |
| `trinity/phase1b_energy_implicit/run_energy_implicit_phase.py` | — | — | ✅ refreshes cube | 🟢 |
| `trinity/bubble_structure/bubble_luminosity.py` | — | — | calls `get_dudt` | 🟢 |
| `trinity/_output/cloudy/snapshot_to_deck.py` | — | — | — | 🟢 |
| plotting (now `paper/figures/`, formerly `src/_plots/`) | — | — | — | 🟢 |
...
1. Replace the OPIATE filename grammar with a `path_cooling_nonCIE` knob
   ...
   from `trinity/cooling/non_CIE/read_cloudy.py`.
...
4. Replace the literal `5.5` boundary and `1e4` floor in
   `trinity/cooling/net_coolingcurve.py` with values derived from the loaded
...
- Not rewriting `net_coolingcurve.get_dudt`'s CIE/non-CIE dispatch logic
  ...
  routing (`trinity/cooling/net_coolingcurve.py:102-158`) is correct as-is.
...
(`trinity/cooling/non_CIE/read_cloudy.py:224, 241`), and NaN ≠ NaN under
plain equality.
...
`trinity/cooling/non_CIE/read_cloudy.py`. Relocate the OPIATE filename
grammar behind a legacy-fallback resolver, mirroring the SPS
`_resolve_sps_bundle` resolver pattern in `trinity/_input/registry.py`
(the SPS refactor never introduced a `_get_legacy_sb99_filename` helper —
it resolves to a single hardcoded default file keyed on `SB99_rotation`/
`ZCloud`). When `path_cooling_nonCIE` resolves to a single file, bypass
age dispatch entirely and load it directly. The CIE side, all loaders'
column layouts, and `net_coolingcurve.py` are untouched.

**Files touched.**

- `trinity/_input/registry.py` — new `_resolve_nonCIE_paths(params,
  age)` resolver (alongside the existing `_resolve_path_cooling_nonCIE`)
  that either (a) returns the user-supplied single file
  verbatim, or (b) runs the relocated OPIATE grammar over the folder.
- `trinity/cooling/non_CIE/read_cloudy.py` — drop the `SB99_rotation` and
  `ZCloud` reads from `get_coolingStructure`
  (`trinity/cooling/non_CIE/read_cloudy.py:47-48`); change `get_filename`
  call at line 59 to `_resolve_nonCIE_paths(params, age)`. Delete
  `get_filename` and `get_fileage` from this module (now live in
  `registry.py`).
- `trinity/_input/default.param` — update the `# INFO` block on
  `path_cooling_nonCIE` to document the single-file user mode. Do
  **not** mark anything deprecated.

**Code-level checklist.**

- [ ] `trinity/cooling/non_CIE/read_cloudy.py` no longer references
      `SB99_rotation` (a `grep -n "SB99" trinity/cooling/` returns nothing).
...
- [ ] The silent `NameError` on unsupported ZCloud
      (`trinity/cooling/non_CIE/read_cloudy.py:290-295`) is replaced by an
...
(`trinity/cooling/non_CIE/read_cloudy.py:182-187`) become the legacy
...
- `trinity/cooling/non_CIE/read_cloudy.py` — refactor `create_cubes` to
  ...
  hash.
- A new `trinity/cooling/cool_columns.py` module — analog of
  `trinity/sps/sps_columns.py` — that owns:
...
`archive/sb99-refactor-audit.md §10 PR-2` checklist applies in spirit.)
...
- [ ] Sign auto-flip at
      `trinity/cooling/non_CIE/read_cloudy.py:189-194` removed; the
...
table. Hardcoded `np.loadtxt(cooling_path, unpack=True) → (logT, logLambda)`
at `trinity/main.py:165` becomes the legacy preset, used when
...
- `trinity/_input/default.param` — `cool_col_cie_*` documentation block.
- `trinity/_input/read_param.py:397-414` (the inline CIE branch in Step 7)
  — detect path-vs-int at line 408;
  if a path-looking string, use it; else apply the legacy index table.
  Replace the silent if/elif fall-through (no `else` raise) with an explicit
  `ValueError` naming both the unsupported `ZCloud` and the available
  legacy combinations.
- `trinity/main.py:162-173` — extract the CIE-load block into
  `load_cie_table(params)` that respects the column map.
- `trinity/cooling/cool_columns.py` — extend with `CANONICALS_CIE`
...
- `trinity/cooling/CIE/read_coolingcurve.py` — drop the unused `metallicity`
  arg on `get_Lambda`
  (`trinity/cooling/CIE/read_coolingcurve.py:25`); update the `get_Lambda`
  docstring's "Available libraries" block to reference the column map /
  path knob instead of the legacy integer table.
- `trinity/cooling/net_coolingcurve.py:88, 148` — drop the
  `params_dict['ZCloud'].value` argument to `CIE.get_Lambda`.

**Files explicitly NOT touched.**

- `trinity/_input/default.param` — `path_cooling_CIE 3`, `path_cooling_nonCIE
  def_dir`, the integer-index preset, and the OPIATE filename grammar
  all remain.
- `trinity/_input/read_param.py` — legacy resolution branches all remain.
...
2. `cool_col_*`-based configs continue to work; full E2E equivalence
   against PR-3 golden.
3. `grep -n "metallicity" trinity/cooling/CIE/read_coolingcurve.py`
   returns only docstring references.
4. `grep -n "ZCloud" trinity/cooling/net_coolingcurve.py` returns nothing.
...
2. **Loader-layer capture (in-process).** Import
   `trinity.cooling.non_CIE.read_cloudy`; for each OPIATE age present in
...
def test_no_SB99_reads_in_read_cloudy():
    ...
    src = open('trinity/cooling/non_CIE/read_cloudy.py').read()
...
def test_get_Lambda_signature_dropped_metallicity():
    """trinity/cooling/CIE/read_coolingcurve.py::get_Lambda has 2 params,
    not 3, after PR-4."""

def test_net_coolingcurve_drops_ZCloud():
    """grep -n 'ZCloud' trinity/cooling/net_coolingcurve.py returns nothing."""
```

### 11.3 Equivalence tolerance policy

Identical to SPS refactor §11.3 (`archive/sb99-refactor-audit.md`), with one
addition: NaN-equality for the non-CIE cubes
(`np.array_equal(..., equal_nan=True)`). The cubes are NaN-initialized
and sparse-filled (`trinity/cooling/non_CIE/read_cloudy.py:224, 241`), so
...
| Path-resolution drift in the relocated OPIATE grammar (mantissa formatter `format(age, '.2e')` at `trinity/cooling/non_CIE/read_cloudy.py:310, 316, 321`) | Medium | High | Path-resolution matrix test (`test_nonCIE_filename_resolution_matrix`) covers all legal combos. |
...
| `cool_col_*` parser collides with the `cool_alpha/beta/delta` physics knobs (`trinity/_input/default.param:279-285`) | Low | Medium | Prefix is `cool_col_cie_` / `cool_col_nonCIE_`, not `cool_`. Adding a test that exercises a config setting both `cool_col_*` and `cool_alpha` confirms no collision. |
...
| NaN-sparse cube comparison changes due to floating-point reordering of `cool - heat` | Low | Medium | `cool_cube - heat_cube` (`trinity/cooling/non_CIE/read_cloudy.py:130`) is element-wise; numpy is deterministic. Test compares NaN-aware. |
...
3. **Anchor configs adequacy.** The three `cloud_example_*` configs
   ...
   from `trinity/_input/default.param:62`. None of them exercise the
...
5. **Shared canonical/unit infrastructure.** `trinity/cooling/cool_columns.py`
   would duplicate ~80% of `trinity/sps/sps_columns.py`'s structure
   (`ColumnSpec`, unit-table dispatch, `_scan_layout`, `load_user_columns`).
   Two options: (a) extract a shared `trinity/_input/column_map_common.py`
...
- **Snapshot exclusion.** All six `cStruc_*` entries are
  `exclude_from_snapshot=True` (declared as `ParamSpec`s at
  `trinity/_input/registry.py:400-405`; listed in
  `trinity/_input/dictionary.py:1171-1177`).
- **Footprint.** Three Python files contain non-trivial loader code:
  - `trinity/cooling/non_CIE/read_cloudy.py` (361 LOC) — the big one.
  - `trinity/cooling/CIE/read_coolingcurve.py` (66 LOC) — trivial.
  - `trinity/cooling/net_coolingcurve.py` (166 LOC) — consumer; touches
    constants only.
...
- **Cooling-update cadence asymmetry.** Phase 1 uses `5e-2 Myr`
  (`trinity/phase1_energy/run_energy_phase.py:56`); phase 1b
  implicit uses `5e-3 Myr`
  (`trinity/phase1b_energy_implicit/run_energy_implicit_phase.py:108`).
  Intentional or accidental? Flagged for review but out of scope.
- **Logging interface.** `logger_cooling =
  logging.getLogger('trinity.cooling.net_coolingcurve')` already exists at
  `trinity/_functions/logging_setup.py:514`. The relocated OPIATE
  resolution helper should log under the `trinity._input` logger
  (`read_param` / `registry`).
...
- **`SB99_rotation`'s home after PR-1.** It stops being read by
  `trinity/cooling/non_CIE/read_cloudy.py` but is still read by the
  relocated OPIATE-grammar resolver (legacy fallback). It also continues
