...
| 9 | `trinity/cooling/net_coolingcurve.py:85` | `if T < 1e4: T = 1e4` — floor hardcoded to the OPIATE non-CIE table's `T_min`. | PR-2: floor to `min(cStruc_cooling_nonCIE.temp)` instead of the constant. |
