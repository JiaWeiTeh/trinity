...
- **`switchon-successor/`** — [`PLAN.md`](switchon-successor/PLAN.md): given that the ramp cannot
  ...
  500-87,000× shorter). 🔵 **pre-registered only.** Bar N3 forbids swapping in another absolute
  constant; bar N1 judges candidates against Weaver Eq. 20 instead of mere equivalence; "keep it
  and justify it better" is a registered outcome.
