...
| `phase1a-init/` | 🔵 ACTIONABLE — M43-scale early-phase artifact fixed and gated on branch `hotfix/early-approximations` (log-spaced phase-1a segments + `vd=-1e8` override deleted); G1a/G1b/G2-vs-reference/G3/eps PASS, but the pre-registered stock-vs-fixed bar is missed at 3e3 yr and 2 golden tests await a re-baseline decision | `PLAN.md`, `data/gate_results.csv` | 2026-08-04 |
