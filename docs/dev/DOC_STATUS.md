...
| `dictionary-robustness/` | 🟡 PARTIAL — snapshot-machinery audit @ `030b658`, **batteries A–G executed and landed** as 67 characterization tests (65 default + 2 stress) (`test_dictionary_stress{,_process}.py`; they pin *current* behavior, defects included). 13 findings probe-verified (F1 boundary-disarmed duplicate guard **confirmed**; F6 flush-retry id shift; F7 load-time clobber of the termination reason; F5 four `save_snapshot` crash modes) + 5 inherited (F14–F18) from the off-trunk 17-finding audit on `fix/audit-dictionary-system` @ `e554316f`, reconciled in `PLAN.md` §1b (unlanded, old-layout, numbers not quotable) + **3 from execution (§1c)**: F19 F5 is reachable from a real `read_param` dict (production survives only because phase 0 fills the arrays before the first save), F20 the crash is in the R² *diagnostic* not the downsampler (cheapest fix, diagnostics-only), F21 F1×F4 ⇒ ~1 phase boundary in 10 duplicates instead of suppressing. O5/O6 resolved (signal reason **is** clobbered by atexit; off-main-thread construction raises). Field scan: fast config clean on I1/I3/I4 but **97/97 lines carry `NaN`**, and F4's 5.0e-4 Myr boundary signature reproduces the old audit exactly. **Three fixes shipped, each gated bit-identical** (`sha256(dictionary.jsonl)` = `17370033…`, 97 snapshots, separate processes): **§1d F20** — the empty-curve R² guard, diagnostics-only, resolving the reachable phase-0 crash F19/F5a/F5d; **§1e F7** — loader-built dicts skip crash-handler registration, so reading a run no longer rewrites its `metadata.json` or hijacks the process's SIGINT/SIGTERM handlers (**I6 now holds**; safe because no production code calls the loaders); **§1f F6** — the flush append is now transactional (serialize-first + truncate-back rollback). ⚠️ **F6's re-verification corrected its own §1 write-up twice** and is the cautionary tale of this workstream: the consequence was *understated* (production retries a poisoned flush 4× — periodic, `main.py:355`, `write_termination_report`, atexit — each swallowed, so the clean prefix was duplicated once per retry, measured `t = [0.0, 1.0, 1.0, 1.0, 1.0]`, and the failing snapshot never landed) while the *trigger* it named was **unreachable** (every interpolator key carries `exclude_from_snapshot`; the live trigger is a torn I/O write, e.g. a full disk). All three fixes re-baselined their crash pins as designed. **Still open:** F1/F21's boundary lottery, F11's bare NaN literals, F5b/F5c, F13's loader id shift, **O1** (an *explicit* save on a loaded dict still deletes the source — the audit's `_readonly`/`_fresh_run` half), **I9's other half** (a signal's reason is still overwritten by atexit), and **I7's other half** (a failed flush still silently *loses* its buffered snapshots; `main.py` only logs it). Everything else stays characterized-not-fixed at `PLAN.md` §6. **Owed:** battery H + F20's byte-comparison on the `f1edge_*` configs, and an all-four-phase run | `README.md`, `PLAN.md` | 2026-08-17 |
