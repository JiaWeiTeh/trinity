...
- **Phase-1a segment-integrator stiffness** → `phase1a-stiffness/PLAN.md` (🔵 Batches 1-4 done —
  solver swap out on economics; the grind is stiffness at collapsed `Eb`; the in-band energy-floor
  event clears P0 and is byte-identical on all five configs, so it lands at Batch 5). Also the
  sole re-open path for magic-number #2 (`dt_switchon`), as its Batch 6.
