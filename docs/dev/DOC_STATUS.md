...
| `phii-identity/` | 🔵 ACTIONABLE — Batches 0/1/4a done. Cap binds on 100% of rows in every phase (6 configs, 4 decades of nCore); `P_HII` == confining pressure to <=2.9e-16. Batch 1's shadow diagnostic `n_IF_Str_raw` is bit-identity gated (593 rows, 3 configs) and showed max blow-up 3.33x, so the C2a kill bar did not trip. **Batch 4a removed the cap: survives cleanly on 4/4 (no distress, no fate changes, faster than baseline) but shifts trajectories 15-28%, over the 5% bar everywhere** — safe but not neutral; landing it is now a physics call (D2). Also measured: momentum drives on exactly 2x P_ram, transition median 1.82x, and `P_HII` carries the un-ramped bubble pressure past the dt_switchon R1 ramp (<=3.2x) | `README.md`, `PLAN.md` | 2026-08-12 |
