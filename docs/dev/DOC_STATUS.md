...
| `dictionary-robustness/` | 🔵 ACTIVE — snapshot-machinery edge-case audit @ `030b658`: 13 findings probe-verified (F1 boundary-disarmed duplicate guard confirmed; F6 flush-retry id shift; F7 load-time clobber of the recorded termination reason; F5 four `save_snapshot` crash modes) + 5 inherited (F14–F18) from the off-trunk 17-finding audit on `fix/audit-dictionary-system` @ `e554316f`, reconciled in `PLAN.md` §1b (that audit is unlanded, old-layout, and its numbers are not quotable); batteries A–H specified, not yet executed | `README.md`, `PLAN.md` | 2026-08-17 |
