...
| `phase1a-init/` | 🔵 ACTIONABLE — M43-scale early-phase artifact fixed and gated on branch `hotfix/early-approximations` (age-scaled phase-1a segments via `phase1a_segFrac` + `vd=-1e8` override deleted); all gates PASS against the G2 bar adopted 2026-08-05 (`|ΔR2| < 10%` at 1 Myr or end of run + fate unchanged), arms converge to −0.001% at 2 Myr, and the fix runs 16% faster; three goldens on the stock 1a exit state re-baselined | `PLAN.md`, `data/gate_results.csv` | 2026-08-05 |
