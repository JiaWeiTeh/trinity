...
| `phii-identity/` | 🔵 ACTIONABLE — Batches 0/1/3/4a done; D1/D2 answered; **an independent audit (2026-08-13) corrected 2 critical + 11 major items — read PLAN.md §9 before quoting any figure from an earlier revision.** Cap binds on 100% of rows in every phase; `P_HII` == confining pressure to <=2.9e-16. Pre-cap blow-up reaches 7.79x (WW), under the 1e2 C2a kill bar. **Batch 4a (cap removed, 4 of core-6): no distress, no fate changes, but trajectories shift 15-28% and the identity is NOT restored** — uncapped `P_HII` still tracks `Pb` (per-config slope +1.00..+1.10, never below `Pb`), because the coupling runs through the ionised volume, not the cap. **Batch 3 (C1 = momentum `max` instead of `+`): safe and small (dR2 0.000-4.00% over weak-wind/2 masses/2 radii, no fate changes) but wrong-target — it deletes the photoionised channel instead of decoupling it.** Next: offline C3a/C3b decoupling screen | `README.md`, `PLAN.md` | 2026-08-13 |
