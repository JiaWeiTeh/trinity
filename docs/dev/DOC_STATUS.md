...
- **Phase-1a segment-integrator stiffness** → `phase1a-stiffness/PLAN.md` (🟡 Batches 1-5 done —
  the in-band energy-floor event is landed on the branch and byte-identical on all five configs;
  **open: a ruling on P3's mypy clause**, §2 D5). Also the sole re-open path for magic-number #2
  (`dt_switchon`), as its Batch 6 — now unblocked.
