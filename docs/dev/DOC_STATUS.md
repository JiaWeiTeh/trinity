...
| `phii-identity/` | 🔵 ACTIONABLE — Batches 0/1/3/4a + 5-stage-1/1b done; D1/D2 answered; independent audit (2026-08-13) corrections in PLAN.md §9. Identity holds to <=2.9e-16, cap binds on 100% of rows; the coupling is the ionised volume, not the cap (removal leaves `P_HII` tracking `Pb`, slope +1.00..+1.10). Double-count measured (momentum 2x, transition 1.82x) and priced by Batch 3 (C1: <=4.0% dR2, safe but wrong-target). **Batch 5: C3b rejected (no `Qi` dependence — fails the wind-only limit structurally); C3c designed (§3c) and screened — a regime switch that transmits while `P_C3a <= P_conf` and drives at `P_C3a` above. Screen: implicit exactly untouched, D-ramp fixed as a side effect, handover crossover inside transition in all 4 configs reaching it, momentum drive 2.4-4.3x stock. C3c supersedes C3a; stage 2 = run arm** | `README.md`, `PLAN.md` | 2026-08-13 |
