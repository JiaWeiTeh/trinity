...
| `phii-identity/` | 🔵 ACTIONABLE — Batches 0/1/4a done; D1/D2 answered; **an independent audit (2026-08-13) corrected several headline numbers — read PLAN.md §9 before quoting any figure from an earlier revision.** Cap binds on 100% of rows in every phase (6 configs, 4 decades of nCore); `P_HII` == confining pressure to <=2.9e-16. Batch 1's shadow diagnostic is bit-identity gated (593 rows) and the pre-cap blow-up reaches **7.79x** (WW), well under the 1e2 C2a kill bar. Batch 4a removed the cap on **4 of the pre-registered core-6** (SC, WW not run): no distress, no fate changes, wall times comparable, but trajectories shift 15-28% and the identity is NOT restored — uncapped `P_HII` still tracks `Pb` (per-config slope +1.00..+1.10, never below `Pb`), because the coupling runs through the ionised volume, not the cap. **C1 has never been run and carries no verdict**; Batch 3 is measuring it | `README.md`, `PLAN.md` | 2026-08-13 |
