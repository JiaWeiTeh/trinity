...
| `phii-identity/` | 🔵 ACTIONABLE — Batches 0/1/3/4a and 5-stage-1 done; D1/D2 answered; **an independent audit (2026-08-13) corrected 2 critical + 11 major items — read PLAN.md §9 before quoting any figure from an earlier revision.** Identity holds to <=2.9e-16 with the cap binding on 100% of rows in every phase. **The cap is not the coupling**: Batch 4a removed it and `P_HII` still tracks `Pb` (per-config slope +1.00..+1.10, never below `Pb`) — the coupling runs through the ionised volume, so the intervention point is the shell ODE's inner BC, not the cap. Double-count measured (momentum exactly 2x P_ram, transition 1.82x median) and priced by Batch 3: removing it costs <=4.0% dR2, no fate changes. **Batch 5 stage 1 (offline): C3b rejected — no Qi dependence, fails the pre-registered wind-only limit structurally; C3a advances to a run arm but predicts a photoionisation-dominated momentum phase (3.5-7.6x P_ram). C3c unevaluated** | `README.md`, `PLAN.md` | 2026-08-13 |
