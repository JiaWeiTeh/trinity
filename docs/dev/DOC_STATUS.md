...
| `switchon-successor/` | 🔵 ACTIONABLE — pre-registered only, no `trinity/` line touched. **Deletion of `dt_switchon` is settled (NO); this asks about its FORM** — the fixed 1e-3 Myr clock runs 500-87,000× longer than `dt_phase0`, the establishment time the code itself computes. Four candidates incl. a root-cause one (inconsistent initial condition); bar **N3 forbids replacing it with another absolute constant**, and bar **N1 judges successors against Weaver Eq. 20** as a *wind-only limiting reference* (§0.3: radiation supplies 32-60% of the early drive, so TRINITY's solution is not Weaver's — the bar is comparative, not "must match"). **Batches 1-3 done:** the drain is **PdV work** (cooling 0.1-0.8%) and the seed violates Weaver's work partition by 4.85× while satisfying its energy exactly (D1); the physical-clock candidate **failed N0 on 3 of 5**, and since the failures are not ordered by window shortening (87,055× survives, 7× dies) **the whole "better clock" family is retired** (D2); the sustainability cap S2 — a limiter with no free constant — became the **first candidate to clear the fate bar on all five** and self-selected release at ≈3× each run's own `dt_phase0`, but **failed N1 on all five** because capping at "no net energy loss" pins `dEb/dt≈0`, so `Eb` plateaus and `Eb/t` decays by construction, landing ~2× further from the reference than the shipped ramp — **S2 out, and the limiter family with it** (D3). **Batch 4 (S4) RUNNING**; its committed pre-registration already shows the handover work rate is `PdV/Lmech = 2(v2/v_wind)/(R1/R2)^2` with **`E0` absent**, so a consistent seed can only mean a slower handover velocity. After it only **S0** (keep-and-justify, backed by D1-D4) remains | `PLAN.md`, `data/s2_state_trigger.csv`, `data/s4_seed_anatomy.csv` | 2026-08-06 |
...
- **`dt_switchon`'s form (not its existence)** → `switchon-successor/PLAN.md` (🔵 Batches 1-3 done,
  **Batch 4 (S4, the consistent seed) RUNNING**): can the fixed 1e-3 Myr clock become a scale-free
