...
| `phii-identity/` | 🟡 **C3c SHIPPED 2026-08-14 (`c43a50e`, PR #738) — one branch still open.** Batches 0/1/3/4a + 5 stages 1/1b/2/3 done; D1-D4 answered; independent audit corrections in PLAN.md §9. Identity holds to <=2.9e-16, cap binds on 100% of rows; the coupling is the ionised volume, not the cap. Double-count measured (momentum 2x, transition 1.82x); C1 priced it at <=4.0% dR2 but is wrong-target. C3b rejected (no `Qi` dependence). **C3c — a regime switch that transmits while `P_C3a <= P_conf` and drives at `P_C3a` above — passed its run arm: 5/5 configs, zero numerical distress, NO fate changes, dR2 12.8-20.5% (pre-registered), null passed exactly (`P_HII`=0 on 0/330 implicit rows). The offline screen predicted the self-consistent regime structure to the printed digit on 3/5 configs.** It now runs in production at all six `P_HII` call sites across the four phase runners. **Stage 3 split the verdict: transition passes** the pre-registered wind ladder (`ratio@entry` 0.7144/0.1227/0.0553/0.0235 vs predicted 0.68/0.12/0.054/0.022, 2-7% per rung) while **momentum stays open** — 100% HII-dominated on all four rungs, `P_C3a/P_ram ∝ Lw^−0.33` ⇒ inversion only at an unphysical `Lw ≈ 260`; the registered dichotomy is recorded **mis-specified**, and the suspect is the `R2^−3/2` cavity geometry, not the prefactor. **Landing consequence (2026-08-14):** C3c fixes D-ramp as a side effect (predicted at PLAN.md §3c; the defect is defined at §3 item 3, whose separate "removing the cap drops the drive" sentence stays retracted — that was about C1, not C3c), so the energy phase now drives on the ramped `Pb` alone and two trajectory goldens are red pending re-baseline (`test_run_smoke` R2 0.259560 → 0.256722; `test_phase_boundary` `cool_beta` 0.888197 → 0.878395, both −1.1%). D4 already granted re-baseline authority for `test_phase_boundary` (and `test_betadelta_hybr_stress` / `test_scheme_screen` fixtures) conditional on G3.4's before/after table; **`test_run_smoke` is not on that list and needs its own sign-off**. Red too, for an unrelated and purely structural reason: the site count in `test_mu_audit_drift` (11 → 5 inline; 6 sites consolidated into `get_phii_c3c`)  **🔴 SHIP-HOLD since 2026-08-18 — do not build further `P_HII` work until D5 is ruled on.** A self-consistency audit (`PLAN.md` §6b) found four driving-branch seams; the maintainer ruled they block shipping **and** that the audit itself must not be assumed correct. **Batch 11 discharged both halves.** **B11.0** re-verified every seam adversarially: photon double-spend **CONFIRMED** (no cavity-absorption factor exists at any `Qi` consumer; `f_abs` = 1.0000 on 29/33 driving rows ⇒ the claimed budget is ≈`2·Qi`), boundary mismatch **REVISED** (real and sized at `P_HII/Pb` median 6.16, but its feedback is exactly zero on 88% of driving rows and *upward* on the rest, so it is not an upper-bound mechanism), mass double-book **CONFIRMED and understated** (0.0952 → **0.5638** by three independent routes, two agreeing to 1e-12, units cleared by `units-reviewer`; and `shell_mass` already holds **100.0000%** of the run's gas while winds inject 54.8 Msun, so `M_cav` has no source and `(M_cav+M_shell)/M_avail` = 1.5638), thin-shell strain **CONFIRMED**. **B11.A–D quantified them and struck two more direction claims**: the photon-conserving fixed point is **degenerate** (unique root `x`=1 on 33/33 rows — cavity takes every photon, shell left neutral), so **C3a cannot be closed photon-conservingly at all**, and repairing the double-spend *raises* `P_C3a` (1.0000–1.1778, 0 rows below 1) rather than lowering it. Only **seam C + G9.4's dust** still bound `P_C3a` from above. **Batch 12** added the missing low-wind rung: both arms of `B3MW01` (`Lw × 0.1`) show the old code returning the confining pressure to **2.2e-16** even with the wind cut 10× — it carries no photoionisation information at any wind strength — against the new code at `P_HII/Pb` = 13.7–14.4, 100% HII-dominated, **+35.1%** in `R2`. ⛔ It also fired a pre-registered falsifier: seam C's 0.564 is a **B3M number, not universal** (`M_cav ∝ R2^{3/2}` ⇒ the seam tracks bubble size, not HII dominance; low wind gives 0.1296), though its qualitative core survives. **D5 is the open decision** — see `PLAN.md` §7.1's K1–K9 candidate register, the single live list. `LITERATURE_ASSESSMENT.md` (external, never load-bearing, C-0 carve-out) is at **revision 2** (2026-08-18): the maintainer's correction landed, withdrew rev1's α_p headline "in full", and its central open question was closed by measurement — TRINITY's `(R2/R1)²` reaches **1.0018** at the 1c→2 handover (committed `b7_regime_trajectory.csv`, both arms), so `α_p = 1` in momentum is delivered by the dynamics, not missing, and the momentum question is entirely `P_HII` (balance volume / closure — K5/K6/K9 in `PLAN.md` §7.1; K7 struck). No `trinity/` source changed in Batches 11–12. | `README.md`, `PLAN.md`, `LITERATURE_ASSESSMENT.md` | 2026-08-18 |
