...
| C2b | **spawned by C2, IN PROGRESS:** `dt_switchon`'s *form* (deletion is settled; the fixed 1e-3 Myr clock vs the `dt_phase0` the code computes). Batches 1-3 done, nothing implemented: the clock family (S1) and the limiter family (S2) are both measured dead. **Next: Batch 4 (S4, the consistent seed)**, then either land it or write S0 (keep-and-justify). **This is the last open item before C2's audit tail** | [J] | `switchon-successor/PLAN.md` |
