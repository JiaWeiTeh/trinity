...
**Status (2026-08-18):** 📘 reference — **revision 2** of the external assessment of TRINITY's
force budget against five papers, contributed by the maintainer. Supersedes revision 1 of the same
date (preserved in git history, last at commit `9aedeb45`); rev2's own §6.2 errata table records
every rev1 claim that fell and why. Retained under `PLAN.md` §0's **C-0 external-document
carve-out**: author's text kept verbatim below, never load-bearing (C-0.5); our corrections and
verifications live only in the cross-check section here.

> **Provenance and standing.** Written 2026-08-18 by an external reviewer with read-only access to
> the tree; revision 2 was run past two independent adversarial passes before release and arrived
> 2026-08-18 as the correction announced by the maintainer — the ⛔ do-not-act banner that stood on
> revision 1 is **lifted** as of this revision. A chapter-formatted `.tex` twin of this document was
> received alongside it and reviewed; the Markdown is committed as canonical (the `.tex` duplicates
> the content, carries thesis styling, and references a figure asset not supplied). **This is not
> the workstream's own measurement**: every claim below remains subject to the ⚠️ banner, and
> nothing in §8's ledger may cite it as evidence.

## B11/B12 cross-check of REVISION 2 — 2026-08-18, written the day it arrived

Nothing in the author's text below was altered. What this workstream did with rev2's new claims:

**Independently verified against source and by derivation (✅), same day:**
- **`run_transition_phase.py:331` is reporting-only** — it sits in `compute_forces_pure` (def at
  `:271`), while the transition RHS delegates to `get_ODE_Edot_pure` (`:231`). Rev2's pass-2
  refutation of its own draft is correct, and it also corrects **this workstream's** D5 site-scope
  text, fixed in `PLAN.md` §7 (dated). Same pattern holds in momentum: `:265` is in
  `compute_forces_momentum_pure`, the live site is `get_ODE_momentum_pure`'s `:445`.
- **The 1c→2 handover is `Eb < ENERGY_FLOOR = 1e3`** (`run_transition_phase.py:97,768`), not the
  `θ = 0.95` trigger — rev1's `eq:tofap` α_p estimator was built on the wrong threshold, as rev2
  says (erratum 15).
- **`params['R1']` is read in exactly one place** (`get_bubbleParams.py:115`, energy/implicit
  path), so momentum's `R1 = R2` is bookkeeping, not dynamics — rev2's pass-2 claim, confirmed.
- **Erratum 2 (the α_p convention reversal) is exact**: with `R_f = (√3/2)·R1` at equal bubble
  pressure, `eq:alphap_shock` equals `(R2/R1)² + (3/16)(R1/R2)²` to 1e-12 — rev1's "4/3 mismatch"
  warning was backwards, and `(R2/R1)²` is the right leading-order quantity to emit.
- **Erratum 8 is exact**: the confinement crossover shifts as `f_abs⁻¹`, outward (numeric
  root-find; rev1's `f_abs^{1/3}` was wrong in exponent and sign).
- The CEM identities verified for rev1 (`harness/cem_closure_check.py`: switch point = `R_ch` to
  4.3e-15; asymptotes/crossover table to the digit) are unchanged by rev2 and now carry rev2's
  correct qualifier — evaluated at `α_p = 1`, the value the momentum phase itself asserts.

**⛔ Rev2's central open question is now MEASURED, and the clean branch wins.** Rev2 §4.4 asks:
*"does TRINITY's α_p collapse from `(R2/R1)² ≫ 1` to 1 too abruptly at the 1c→2 handover?"* From
the committed `data/b7_regime_trajectory.csv` (B3M, both arms, `t > dt_switchon` only):
`(R2/R1)²` runs **8.4–12.2** (energy) → **12.9–44.7** (implicit) → **37.9 → 1.0018** (transition),
reaching **1.002 exactly as `Eb` hits the floor** — c3c arm 1.0018, stock arm 1.0017. There is
**no discontinuity**: TRINITY's own dynamics delivers the wind-force enhancement to within 0.2% of
the momentum phase's asserted value before the phase switch. Consequences: (i) rev2's withdrawal of
the α_p knob is now measurement-backed, not just argument-backed; (ii) the momentum-phase
HII-dominance question is **entirely about `P_HII`** — the balance volume and closure — exactly
where Batch 11 already placed it. (Recipe: ratio of the committed `R2`,`R1` columns, squared,
excluding `t ≤ 1e-3`; no run needed.)

**One number rev2 removed as unsourceable is in fact sourced (give-back):** the 0.487 lower bound
on Batch 7's confinement ratio is `ratio_min = 0.4873` on the `B3MW001` energy row of
`data/b7_confinement_screen.csv` — the file was simply absent from the author's staged copy.

**Still unmeasured, correctly flagged by rev2:** whether TRINITY's `(R2/R1)²` (8–45 in its own
energy/implicit phases, vs Paper II's tabulated 4.57–6.82) should be read as under-dissipation —
that comparison needs the θ bookkeeping done carefully and is B11.G-adjacent, not settled here.
