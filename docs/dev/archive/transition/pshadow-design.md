...
**About this document**
- **Status (updated 2026-06-22):** ⛔ **SUPERSEDED** by `docs/dev/transition/cleanroom/FINDINGS.md` — the two-criterion F0 ∨ F4 design rests on "flat configs transition by cooling (F0 fires)", which the 6-config clean-room **falsified** (F0 never fires; even flat `simple_cluster` blows out geometrically). Never built, nothing shipped. Kept as a historical design record.
...
6-config clean-room (`docs/dev/transition/cleanroom/FINDINGS.md`): cooling never fires, so the F0 half
