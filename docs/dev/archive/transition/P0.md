...
**About this document**
- **Status (updated 2026-06-22):** ⛔ **SUPERSEDED** by `docs/dev/transition/cleanroom/FINDINGS.md` — the P0 G0 verdict ("PROCEED with a profile-dependent cooling ∨ blowout trigger") was overturned by the 6-config clean-room baseline: cooling (F0/F1) **never fires in any regime**, so the split is not "flat→cooling, steep→blowout" but **blowout-only**. P-shadow was never built. Kept as the offline results record.
...
Execution log for **P0** of `docs/dev/archive/transition/TRIGGER_PLAN.md`. **Status
