...
| **F1** | stop the 60k dense-output resample in the dMdt residual (`RESAMPLE_PLAN.md`) | 🟢 P0 done (5/6 configs), patch+test staged; fix not yet applied | **uniform ~1.5×/call energy, ~1.4×/call implicit across ALL configs** (degenerate `simple_cluster` 1.52×/1.39× = same as tiny `mock_hybr`); worst `rel_dMdt` **3.1e-6** ≪ 0.3% gate. Per-call win is config-INDEPENDENT (resample is fixed-size); degenerate payoff is a full-run effect (P4) | numerator exact; denominator ~1e-12; coarse vs 20k-dense ≤3e-6 (measured, 5 configs) | `data/master_p0_table.csv`; patch `P3_PRODUCTION_PATCH.md`; guard `test/test_residual_resample.py`; **next: apply P3 + P4 full-run A/B** |
