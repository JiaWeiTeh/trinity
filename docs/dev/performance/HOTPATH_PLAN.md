...
| **F1** | stop the 60k dense-output resample in the dMdt residual (`RESAMPLE_PLAN.md`) | 🟠 P3 applied (`24c6914`); **NOT cleared — full-run edge validation OPEN; revert-ready** | per-call win uniform ~1.5×/call (E) / ~1.4×/call (I), worst per-call `rel_dMdt` 3.1e-6 — **but per-call is necessary-not-sufficient.** Full-run: `mock_hybr` matches 60k to ~5e-6; stiff edge cases re-running (the decider) | 60k does real `min_T`/monotonic convergence work on sharp T — coarse `t_eval` can under-resolve it; per-call gate can't see this | `data/master_p0_table.csv`; `test/test_residual_resample.py`; edge configs `docs/dev/performance/f1edge_*.param`; **next: edge full-run verdict → ship or revert** |
