...
| **F1** | stop the 60k dense-output resample in the dMdt residual (`RESAMPLE_PLAN.md`) | 🟡 P0 validated on `mock_hybr`; fix unwritten | **~1.5× faster/bubble-call** on `mock_hybr` (baseline 1362 ms → variants 825–912 ms; degenerate configs expected larger); worst `rel_dMdt` **1.0e-06** ≪ 0.3% gate | numerator exact; denominator ~1e-12; coarse-sample within 1e-6 (measured) | `performance/harness/` + `data/bubble_resample_mock_hybr.csv`; full 6-config sweep pending |
